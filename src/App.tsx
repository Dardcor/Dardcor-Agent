...
const App: React.FC = () => {
  ...
  const navItems = [
    ...
    { id: 'background', path: '/background',    label: 'Background Tasks', icon: '🔁' },
    { id: 'mcp',        path: '/mcp',           label: 'MCP Servers',      icon: '🔌' },
    { id: 'plugins',    path: '/plugins',       label: 'Plugins',          icon: '🧩' },
    { id: 'stats',      path: '/stats',         label: 'Stats & Index',    icon: '📈' },
  ...
  const headerTitle: Record<string, string> = {
    chat: 'Agent Chat', model: 'Model Settings', tools: 'Tools', skills: 'Skills',
    background: 'Background Tasks', mcp: 'MCP Servers', plugins: 'Plugins', stats: 'Stats & Cost',
    explorer: 'File Explorer', terminal: 'Terminal', system: 'System Monitor', workspace: 'Workspace',
  }
      ...
      <main className="main-content">
        <header className="main-header">
          <div className="header-title">
            ...
            <h2>{headerTitle[activeTab] || activeTab}</h2>
            <SentienceMonitor />
          </div>

          <div className="header-actions">
            <Link to="/chat" className={`header-btn ${activeTab === 'chat' ? 'active' : ''}`}>Chat</Link>
            <Link to="/terminal" className={`header-btn ${activeTab === 'terminal' ? 'active' : ''}`}>Terminal</Link>
            <Link to="/background" className={`header-btn ${activeTab === 'background' ? 'active' : ''}`}>Tasks</Link>
          </div>
        </header>
        ...
        <div className="tab-content">
          <Routes>
            ...
            <Route path="/background" element={<BackgroundTasksPanel />} />
