...
# 套件管理員
npm i -g dardcor-ai@latest        # 也可使用 bun/pnpm/yarn
scoop install dardcor             # Windows
choco install dardcor             # Windows
brew install anomalyco/tap/dardcor # macOS 與 Linux（推薦，始終保持最新）
brew install dardcor              # macOS 與 Linux（官方 brew formula，更新頻率較低）
sudo pacman -S dardcor            # Arch Linux (Stable)
paru -S dardcor-bin               # Arch Linux (Latest from AUR)
mise use -g dardcor               # 任何作業系統
nix run nixpkgs#dardcor           # 或使用 github:anomalyco/dardcor 以取得最新開發分支
```
...
dardcor 也提供桌面版應用程式。您可以直接從 [發佈頁面 (releases page)](https://dardcor-code.vercel.app/) 或 [dardcor.ai/download](https://dardcor-code.vercel.app/) 下載。
...
1. `$dardcor_INSTALL_DIR` - 自定義安裝目錄
2. `$XDG_BIN_DIR` - 符合 XDG 基礎目錄規範的路徑
3. `$HOME/bin` - 標準使用者執行檔目錄 (若存在或可建立)
4. `$HOME/.dardcor/bin` - 預設備用路徑
...
dardcor 內建了兩種 Agent，您可以使用 `Tab` 鍵快速切換。
...
此外，dardcor 還包含一個 **general** 子 Agent，用於處理複雜搜尋與多步驟任務。此 Agent 供系統內部使用，亦可透過在訊息中輸入 `@general` 來呼叫。

了解更多關於 [Agents](https://dardcor-code.vercel.app/) 的資訊。

### 線上文件

關於如何設定 dardcor 的詳細資訊，請參閱我們的 [**官方文件**](https://dardcor-code.vercel.app/)。

### 參與貢獻

如果您有興趣參與 dardcor 的開發，請在提交 Pull Request 前先閱讀我們的 [貢獻指南 (Contributing Docs)](./CONTRIBUTING.md)。

### 基於 dardcor 進行開發

如果您正在開發與 dardcor 相關的專案，並在名稱中使用了 "dardcor"（例如 "dardcor-dashboard" 或 "dardcor-mobile"），請在您的 README 中加入聲明，說明該專案並非由 dardcor 團隊開發，且與我們沒有任何隸屬關係。

---

**加入我們的社群** [飞书](https://dardcor-code.vercel.app/) | [X.com](https://dardcor-code.vercel.app/)
