# Remove `packages/dardcor/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/dardcor/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the dardcor legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/dardcor/src/storage/db.ts` are concentrated in 22 source files:

- `packages/dardcor/src/account/repo.ts`
- `packages/dardcor/src/cli/cmd/db.ts`
- `packages/dardcor/src/cli/cmd/import.ts`
- `packages/dardcor/src/cli/cmd/stats.ts`
- `packages/dardcor/src/control-plane/workspace.ts`
- `packages/dardcor/src/index.ts`
- `packages/dardcor/src/node.ts`
- `packages/dardcor/src/permission/index.ts`
- `packages/dardcor/src/project/project.ts`
- `packages/dardcor/src/server/projectors.ts`
- `packages/dardcor/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/dardcor/src/server/shared/fence.ts`
- `packages/dardcor/src/session/message-v2.ts`
- `packages/dardcor/src/session/projectors.ts`
- `packages/dardcor/src/session/prompt.ts`
- `packages/dardcor/src/session/session.ts`
- `packages/dardcor/src/session/todo.ts`
- `packages/dardcor/src/share/share-next.ts`
- `packages/dardcor/src/storage/db.ts`
- `packages/dardcor/src/sync/index.ts`
- `packages/dardcor/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy dardcor database wrapper; `packages/dardcor/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the dardcor wrapper.
...
Status: Completed. `SyncEvent` and the dardcor projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy dardcor database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy dardcor database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy dardcor database wrapper.
...
All migration groups are complete or superseded. `packages/dardcor/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No dardcor data-migration group remains.

The previous dardcor `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate dardcor data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/dardcor`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/dardcor/src`
- `bun typecheck` from `packages/dardcor`
- Relevant package tests from `packages/dardcor`, not the repo root
