...
Move the canonical dardcor terminal application from
`packages/dardcor/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/dardcor`,
`packages/cli`, or `@dardcor-ai/core`.

The SDK is the TUI's dardcor boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/dardcor`.
- Do not replace private `packages/dardcor` imports with `@dardcor-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@dardcor-ai/tui`.
...
- dardcor domain data displayed by the TUI
...
```text
packages/dardcor/src/cli/cmd/tui
```

Its private dependency on `packages/dardcor` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/dardcor/src/*`.
...
Status: Completed. The private `@dardcor-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@dardcor-ai/tui`.
...
- The package has no dependency on `dardcor`, `@dardcor-ai/cli`, or
  `@dardcor-ai/core`.
...
primitives now live in `@dardcor-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@dardcor-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads dardcor globals or process state;
...
Replace process-global dardcor state with resolved TUI inputs.
...
`@dardcor-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/dardcor` initially.
...
canonical in `@dardcor-ai/tui`. Configured references resolve through the new
...
adapters rather than dardcor domain implementations.

Make the SDK the only dardcor domain boundary used by the TUI.
...
- Domain-facing TUI code imports dardcor data and operations only from
  `@dardcor-ai/sdk`.
...
`@dardcor-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/dardcor`.
...
- The package has no imports from `packages/dardcor`, `packages/cli`, or
  `@dardcor-ai/core`.
...
public `@dardcor-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/dardcor`.
- Keep the legacy embedded worker and server startup in `packages/dardcor`.
- Change those adapters to load config, create transport inputs, and call the
  public `@dardcor-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/dardcor`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@dardcor-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@dardcor-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for dardcor domain state.
...
cd packages/dardcor && bun typecheck
...
rg '@dardcor-ai/core|packages/dardcor|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/dardcor/src packages/dardcor/test
```
...
- Build the current-platform `packages/dardcor` binary.
