...
Both `action` and `resource` use dardcor's existing wildcard matching behavior.
...
The relative policy precedence of direct project files and `.dardcor` files is intentionally deferred until `.dardcor` configuration is reviewed.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether dardcor permits an operation through its managed execution paths.
