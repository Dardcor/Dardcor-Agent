...
The dardcor server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@dardcor-ai/llm` tool loop after replacing its
...
- plugins get global "dardcor" instance like in that post i showed
- dardcor instance has stuff like `dardcor.session.prompt()` or
  `dardcor.tool.register({...})`
...
- decide whether to preserve deprecated `@dardcor-ai/llm` orchestration exports
