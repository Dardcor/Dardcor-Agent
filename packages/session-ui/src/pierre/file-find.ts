...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("dardcor-find")
  api.delete("dardcor-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("dardcor-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("dardcor-find", new Highlight(...rest))
    return true
  }
