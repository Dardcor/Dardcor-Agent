...
- Author package entrypoint is `@dardcor-ai/plugin/tui`.
...
```json
{
  "$schema": "https://dardcor-code.vercel.app//tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/dardcor-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@dardcor-ai/plugin/tui`.
- `@dardcor-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@dardcor-ai/plugin/tui"
...
- The value is a semver range checked against the running dardcor version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.dardcor` is absent, no check is performed (backward compatible).
...
- `dardcor plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `dardcor plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.dardcor` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.dardcor`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.dardcor`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.dardcor` dir or `dardcor_CONFIG_DIR`, dardcor installs `@dardcor-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@dardcor-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus dardcor's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
dardcor registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, dardcor pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@dardcor-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"dardcor"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `dardcor.default`.
...
- Local plugins persist installed themes under the local `.dardcor/themes` area near the plugin config source.
...
- `--pure` / `dardcor_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.dardcor/plugins/tui-smoke.tsx`
- Local vim plugin: `.dardcor/plugins/tui-vim.tsx`
- Local smoke config: `.dardcor/tui.json`
- Local smoke theme: `.dardcor/plugins/smoke-theme.json`
