...
Delete `packages/dardcor/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/dardcor/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/dardcor/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/dardcor`
- TUI plugin loader tests in `packages/dardcor` if runtime plugin API wiring changed
