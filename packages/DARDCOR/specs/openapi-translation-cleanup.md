...
Trim `packages/dardcor/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/dardcor/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/dardcor`.
- `[x]` `bun typecheck` from `packages/dardcor`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/dardcor/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/dardcor`.
- `[x]` `bun dev generate > /tmp/dardcor-openapi.json` from `packages/dardcor`.
...
- Edit `packages/dardcor/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/dardcor-openapi.json` from `packages/dardcor`.
...
- Check source schemas in files like `packages/dardcor/src/session/schema.ts`, `packages/dardcor/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/dardcor/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/dardcor/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
