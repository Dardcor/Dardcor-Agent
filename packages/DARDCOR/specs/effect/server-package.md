...
Practical reference for a future `packages/server` split after the dardcor
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/dardcor`.
...
Until enough shared service code lives outside `packages/dardcor`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/dardcor`

It should not import `packages/dardcor` services while `packages/dardcor`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on dardcor-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/dardcor` implementation details.
