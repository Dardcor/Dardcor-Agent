...
import { SessionMessageTable } from "@dardcor-ai/core/session/sql"
...
import { Shell } from "@dardcor-ai/core/shell"
...
import { LocationServiceMap, locationServiceMapLayer } from "@dardcor-ai/core/location-services"
...
const writeConfig = Effect.fn("test.writeConfig")(function* (dir: string, config: Partial<ConfigV1.Info>) {
  yield* writeText(
    path.join(dir, "dardcor.json"),
    JSON.stringify({ $schema: "https://dardcor-code.vercel.app//config.json", ...config }),
  )
})
...
it.instance("does not loop empty assistant turns for a simple reply", () =>
  Effect.gen(function* () {
    ...
    yield* llm.text("packages/dardcor/src/session/processor.ts")
  ...
  () =>
    Effect.gen(function* () {
      ...
      const other = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        model: { providerID: ProviderV2.ID.make("dardcor"), modelID: ModelV2.ID.make("kimi-k2.5-free") },
        noReply: true,
        parts: [{ type: "text", text: "hello" }],
      })
