...
import type { NamedError } from "@dardcor-ai/core/util/error"
...
describe("session.retry.retryable", () => {
  ...
  test("maps free limits to Go upsell action", () => {
    ...
    expect(SessionRetry.retryable(error, "dardcor")).toEqual({
  ...
  test("maps Go subscription limits to workspace PAYG upsell", () => {
    ...
    expect(SessionRetry.retryable(error, "dardcor-go")).toEqual({
      message:
        "5 hour usage limit reached. It will reset in 5 hours 23 minutes. To continue using this model now, enable usage from your available balance - https://dardcor-code.vercel.app//workspace/wrk_01K6XGM22R6FM8JVABE9XDQXGH/go",
      action: {
        ...
        label: "open settings",
        link: "https://dardcor-code.vercel.app//workspace/wrk_01K6XGM22R6FM8JVABE9XDQXGH/go",
      },
    })
  })
  ...
  test("maps Go subscription limits without limit metadata", () => {
    ...
    expect(SessionRetry.retryable(error, "dardcor-go")?.action?.message).toBe(
