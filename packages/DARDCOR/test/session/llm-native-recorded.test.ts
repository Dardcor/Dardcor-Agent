...
import { SessionV1 } from "@dardcor-ai/core/v1/session"
import { ModelsDev } from "@dardcor-ai/core/models-dev"
import { HttpRecorder } from "@dardcor-ai/http-recorder"
import { HttpRecorderInternal } from "@dardcor-ai/http-recorder/internal"
...
import { LLMEvent, LLMResponse } from "@dardcor-ai/llm"
import { RequestExecutor } from "@dardcor-ai/llm/route"
...
const zenURL = (connection: string) => `https://console.dardcor.ai/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.dardcor_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["dardcor", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("dardcor_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("dardcor_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["dardcor", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "dardcor-proxy",
    name: "dardcor proxy",
    providerID: ProviderV2.ID.dardcor,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["dardcor", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.dardcor_RECORD_CONSOLE_TOKEN && process.env.dardcor_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.dardcor,
        name: "opencode Zen",
        env: ["dardcor_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.dardcor_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.dardcor_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.dardcor_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["dardcor", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("dardcor_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("dardcor_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("dardcor_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set dardcor_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the dardcor LLM stack remain real.
  const metadata = {
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "dardcor.json"),
      JSON.stringify({ $schema: "https://dardcor-code.vercel.app//config.json", ...scenario.config(model) }),
    ),
  )
