...
import type { QuestionRequest } from "@dardcor-ai/sdk/v2"
import { DardcorKeymapProvider, registerdardcorKeymap } from "@dardcor-ai/tui/keymap"
import {
...
function model(input: {
  ...
  return {
    ...
    api: {
      id: "dardcor",
      url: "https://dardcor-code.vercel.app/",
...
async function renderFooter(
  ...
  function Harness() {
    ...
    offKeymap = registerdardcorKeymap(keymap, renderer, config)
...
test("direct footer shows editable prompts and additional queued work while running", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    offKeymap = registerdardcorKeymap(keymap, renderer, tuiConfig)
...
test("direct footer separates a lone context hint from model and command hint", async () => {
  const app = await renderFooter({
    providers: [provider()],
    currentModel: { providerID: "dardcor", modelID: "gpt-5" },
...
test.skip("direct custom answer submits through keymap return binding", async () => {
  ...
  function Harness() {
    const renderer = useRenderer()
    const keymap = createDefaultOpenTuiKeymap(renderer)
    off = registerdardcorKeymap(keymap, renderer, tuiConfig)
...
test("direct model panel renders current model selector", async () => {
  ...
  const [current] = createSignal<RunInput["model"]>({ providerID: "dardcor", modelID: "gpt-5" })
  ...
  try {
    ...
    expect(frame).toContain("Search")
    expect(frame).toContain("dardcor")
    expect(frame).toContain("GPT-5")
