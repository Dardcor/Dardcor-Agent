// Subprocess integration tests for `dardcor run` (non-interactive mode).
// These exercise the real CLI binary against a TestLLMServer running in the
// same process. See `test/lib/cli-process.ts` for the harness — each test uses
// `dardcor.run(message, opts?)` to spawn `bun src/index.ts run ...` with
// `dardcor_CONFIG_CONTENT` providing the test provider config inline.
import { describe, expect } from "bun:test"
import { Effect } from "effect"
import { reply } from "../../lib/llm-server"
import { cliIt } from "../../lib/cli-process"

describe("dardcor run (non-interactive subprocess)", () => {
  ...
  cliIt.concurrent(
    ...
    ({ llm, dardcor }) =>
      Effect.gen(function* () {
        ...
        const result = yield* dardcor.run("say hi")
        dardcor.expectExit(result, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, dardcor }) =>
      Effect.gen(function* () {
        ...
        const result = yield* dardcor.run("use a tool", {
  ...
  cliIt.concurrent(
    ...
    ({ llm, dardcor }) =>
      Effect.gen(function* () {
        ...
        const thinking = yield* dardcor.run("think", { extraArgs: ["--thinking"] })
        dardcor.expectExit(thinking, 0)
        expect(thinking.stdout).toBe("Thinking: considering\nanswer\n")

        yield* llm.reason("hidden", { text: "visible" })
        const plain = yield* dardcor.run("think again")
        dardcor.expectExit(plain, 0)
  ...
  cliIt.concurrent(
    ...
    ({ llm, dardcor }) =>
      Effect.gen(function* () {
        ...
        const result = yield* dardcor.run("trigger midstream error", { timeoutMs: 30_000 })
  ...
  cliIt.concurrent(
    ...
    ({ llm, dardcor }) =>
      Effect.gen(function* () {
        ...
        const result = yield* dardcor.run("say hi", { format: "json" })
        dardcor.expectExit(result, 0)

        const events = dardcor.parseJsonEvents(result.stdout)
  ...
  cliIt.concurrent(
    ...
    ({ dardcor }) =>
      Effect.gen(function* () {
        const result = yield* dardcor.run("use an unknown model", {
          model: "test/nonexistent-model",
          format: "json",
        })
  ...
  cliIt.concurrent(
    ...
    ({ llm, dardcor }) =>
      Effect.gen(function* () {
        ...
        const result = yield* dardcor.run("exercise json records", {
          format: "json",
  ...
  cliIt.concurrent(
    ...
    ({ llm, dardcor }) =>
      Effect.gen(function* () {
        ...
        const result = yield* dardcor.run("fail after output", { format: "json" })
  ...
  cliIt.concurrent(
    ...
    ({ home, llm, dardcor }) =>
      Effect.gen(function* () {
        ...
        const denied = yield* dardcor.run("request permission", { permission: { bash: "ask" } })
        dardcor.expectExit(denied, 0)
        ...
        const allowed = yield* dardcor.run("request permission", {
          permission: { bash: "ask" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        dardcor.expectExit(allowed, 0)
        ...
        const explicitlyDenied = yield* dardcor.run("request denied permission", {
          permission: { bash: "deny" },
          extraArgs: ["--dangerously-skip-permissions"],
        })
        dardcor.expectExit(explicitlyDenied, 0)
  ...
  cliIt.live(
    ...
    ({ home, llm, dardcor }) =>
      Effect.gen(function* () {
        ...
        const server = yield* dardcor.serve()

        const result = yield* dardcor.run("read the attachment", {
  ...
  cliIt.concurrent(
    ...
    ({ home, dardcor }) =>
      Effect.gen(function* () {
        const result = yield* dardcor.run("read the directory", {
  ...
  cliIt.live(
    ...
    ({ llm, dardcor }) =>
      Effect.gen(function* () {
        yield* llm.hang
        const run = yield* dardcor.startRun("wait forever")
        yield* llm.wait(1)
        run.interrupt()
