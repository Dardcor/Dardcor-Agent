...
import { dardcorClient, type GlobalEvent } from "@dardcor-ai/sdk/v2"
import { createSessionTransport } from "@/cli/cmd/run/stream.transport"
import type { FooterApi, FooterEvent, LocalReplayRow, RunFilePart, StreamCommit } from "@/cli/cmd/run/types"

type EventStream = Awaited<ReturnType<dardcorClient["event"]["subscribe"]>>["stream"]
type GlobalEventStream = Awaited<ReturnType<dardcorClient["global"]["event"]>>["stream"]
type SdkEvent = EventStream extends AsyncGenerator<infer T, unknown, unknown> ? T : never
type SessionMessage = NonNullable<Awaited<ReturnType<dardcorClient["session"]["messages"]>>["data"]>[number]
type SessionChild = NonNullable<Awaited<ReturnType<dardcorClient["session"]["children"]>>["data"]>[number]
type SessionToolPart = Extract<SessionMessage["parts"][number], { type: "tool" }>
type SessionStatusMap = NonNullable<Awaited<ReturnType<dardcorClient["session"]["status"]>>["data"]>
...
function ok<T>(data: T) {
  return Promise.resolve({
    data,
    error: undefined,
    request: new Request("https://dardcor.test"),
...
function sdk(
  input: {
    ...
    subscribe?: dardcorClient["event"]["subscribe"]
    globalEvent?: dardcorClient["global"]["event"]
    promptAsync?: dardcorClient["session"]["promptAsync"]
    status?: dardcorClient["session"]["status"]
    messages?: dardcorClient["session"]["messages"]
    children?: dardcorClient["session"]["children"]
    permissions?: dardcorClient["permission"]["list"]
    questions?: dardcorClient["question"]["list"]
  } = {},
) {
  const client = new dardcorClient()

  const subscribe: dardcorClient["event"]["subscribe"] = input.subscribe ?? (() => sse(input.stream ?? emptyStream()))
  const globalEvent: dardcorClient["global"]["event"] =
    input.globalEvent ?? (() => globalSse(input.globalStream ?? wrapGlobalStream(input.stream ?? emptyStream())))
  const promptAsync: dardcorClient["session"]["promptAsync"] = input.promptAsync ?? (() => ok(undefined))
  const status: dardcorClient["session"]["status"] = input.status ?? (() => ok({}))
  const messages: dardcorClient["session"]["messages"] = input.messages ?? (() => ok([]))
  const children: dardcorClient["session"]["children"] = input.children ?? (() => ok([]))
  const permissions: dardcorClient["permission"]["list"] = input.permissions ?? (() => ok([]))
  const questions: dardcorClient["question"]["list"] = input.questions ?? (() => ok([]))
