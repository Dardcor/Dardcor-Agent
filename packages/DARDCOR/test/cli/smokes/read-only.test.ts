...
describe("dardcor read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // dardcor_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ dardcor }) =>
      Effect.gen(function* () {
        const r = yield* dardcor.spawn(["mcp", "list"])
        dardcor.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ dardcor }) =>
      Effect.gen(function* () {
        const r = yield* dardcor.spawn(["providers", "list"])
        dardcor.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ dardcor }) =>
      Effect.gen(function* () {
        const r = yield* dardcor.spawn(["models"])
        dardcor.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ dardcor }) =>
      Effect.gen(function* () {
        const r = yield* dardcor.spawn(["agent", "list"])
        dardcor.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh dardcor_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ dardcor }) =>
      Effect.gen(function* () {
        const r = yield* dardcor.spawn(["session", "list"])
        dardcor.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ dardcor }) =>
      Effect.gen(function* () {
        const r = yield* dardcor.spawn(["stats"])
        dardcor.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under dardcor_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ dardcor }) =>
      Effect.gen(function* () {
        const r = yield* dardcor.spawn(["db", "path"])
        dardcor.expectExit(r, 0, "db path")
