...
// terminal sizes. The default dardcor tui command is excluded —
// `dardcor --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `dardcor --help` prints today;
...
// top-level help on `--help` and exits 1; not a real dardcor command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("dardcor CLI help-text snapshots", () => {
  ...
  cliIt.live(
    ...
    ({ dardcor }) =>
      Effect.gen(function* () {
        const topLevel = yield* dardcor.spawn(["--help"], { env: SNAPSHOT_ENV })
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* dardcor.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`dardcor ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`dardcor ${argv.join(" ")} --help`)
        }
