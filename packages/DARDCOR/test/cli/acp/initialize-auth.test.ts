...
describe("dardcor acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ dardcor }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ dardcor }))
        ...
        expect(initialized.agentInfo?.name).toBe("dardcor")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ dardcor }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ dardcor })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("dardcor-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "dardcor-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.dardcor_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ dardcor }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("dardcor-login")
