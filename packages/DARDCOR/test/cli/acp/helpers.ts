...
export function createAcpClient(input: Pick<CliFixture, "dardcor">, env?: Record<string, string>) {
  return Effect.gen(function* () {
    return createJsonRpcAcpClient(yield* input.dardcor.acp(env ? { env } : undefined))
  })
}
  ...
  return Effect.gen(function* () {
    return expectOk(
      yield* acp.request<InitializeResponse>("initialize", {
        ...
        clientInfo: { name: "dardcor-local-acp", version: "0.1.0" },
      }),
    )
  })
}
