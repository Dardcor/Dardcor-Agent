...
describe("tui thread", () => {
  ...
  cliIt.live("rejects mini-only options without --mini", ({ dardcor }) =>
    Effect.gen(function* () {
      const result = yield* dardcor.spawn(["--replay-limit", "10"])

      dardcor.expectExit(result, 1)
      expect(result.stderr).toContain("--replay-limit requires --mini")
    }),
  )

  cliIt.live("routes attached sessions to mini mode", ({ dardcor }) =>
    Effect.gen(function* () {
      const result = yield* dardcor.spawn(["attach", "http://127.0.0.1:1", "--mini"])
  ...
  cliIt.live("rejects network options in mini mode", ({ dardcor }) =>
    Effect.gen(function* () {
      const result = yield* dardcor.spawn(["--mini", "--port", "4096"])
