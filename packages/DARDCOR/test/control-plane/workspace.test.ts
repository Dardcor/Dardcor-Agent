...
import { EventSequenceTable } from "@dardcor-ai/core/event/sql"
...
import { WorkspaceV2 } from "@dardcor-ai/core/workspace"
import { WorkspaceTable } from "@dardcor-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  dardcor_AUTH_CONTENT: process.env.dardcor_AUTH_CONTENT,
  dardcor_EXPERIMENTAL_WORKSPACES: process.env.dardcor_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.dardcor_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@dardcor.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.dardcor_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=dardcor-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.dardcor_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.dardcor_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.dardcor_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=dardcor-test")
