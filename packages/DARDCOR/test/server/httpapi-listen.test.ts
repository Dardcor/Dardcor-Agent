...
const original = {
  ...
  envPassword: process.env.dardcor_SERVER_PASSWORD,
  envUsername: process.env.dardcor_SERVER_USERNAME,
}
const auth = { username: "dardcor", password: "listen-secret" }
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.dardcor_SERVER_PASSWORD
  else process.env.dardcor_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.dardcor_SERVER_USERNAME
  else process.env.dardcor_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.dardcor_SERVER_PASSWORD = auth.password
  Flag.dardcor_SERVER_USERNAME = auth.username
  process.env.dardcor_SERVER_PASSWORD = auth.password
  process.env.dardcor_SERVER_USERNAME = auth.username
...
async function startNoAuthListener() {
  ...
  Flag.dardcor_SERVER_USERNAME = auth.username
  delete process.env.dardcor_SERVER_PASSWORD
  process.env.dardcor_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
async function requestTicket(
  ...
  const response = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", id), listener.url), {
    ...
    headers: {
      ...
      "x-dardcor-directory": dir,
      ...(options?.ticketHeader === false ? {} : { "x-dardcor-ticket": "1" }),
...
describe("HttpApi Server.listen", () => {
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-dardcor-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.dardcor_DISABLE_DEFAULT_PLUGINS
    process.env.dardcor_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.dardcor_DISABLE_DEFAULT_PLUGINS
      else process.env.dardcor_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      const ambiguous = await fetch(new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url), {
        method: "POST",
        headers: { authorization: authorization(), "x-dardcor-ticket": "1" },
      })
