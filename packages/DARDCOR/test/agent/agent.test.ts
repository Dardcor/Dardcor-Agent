...
it.instance("plan agent denies edits except .dardcor/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".dardcor/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".dardcor", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.dardcor_TEST_HOME
      process.env.dardcor_TEST_HOME = test.directory
