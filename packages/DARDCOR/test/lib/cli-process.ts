// Subprocess test harness for the dardcor CLI. Spawns the real binary against
...
//
// Configuration flows through dardcor's built-in test affordances:
//   - dardcor_CONFIG_CONTENT      : provider config inline, no files to find
//   - dardcor_TEST_HOME           : pins os.homedir() → tmpdir
//   - dardcor_DISABLE_PROJECT_CONFIG : skip walking up for dardcor.json
//   - dardcor_PURE                : skip external plugin discovery + install
//   - dardcor_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `dardcor.run` is fully wired. The shape supports adding more
// builders (`dardcor.serve(opts)`, `dardcor.acp(opts)`, `dardcor.auth(...)`)
// without changing the fixture. Long-lived commands like `serve` will need a
// different return shape — see the TODO at the bottom of dardcorCli.
...
import { AppProcess } from "@dardcor-ai/core/process"
...
const dardcorRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(dardcorRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    dardcor_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    dardcor_CONFIG_CONTENT: configJson,
    dardcor_DISABLE_PROJECT_CONFIG: "1",
    dardcor_PURE: "1",
    dardcor_DISABLE_AUTOUPDATE: "1",
    dardcor_DISABLE_AUTOCOMPACT: "1",
    dardcor_DISABLE_MODELS_FETCH: "1",
    dardcor_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `dardcor run`. New flags should
...
// `dardcor serve` is a long-lived process — it never exits on its own.
...
// `dardcor acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type dardcorCli = {
  ...
  // Spawn `dardcor serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `dardcor acp` and return a duplex JSON-RPC handle. Long-lived:
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("dardcor.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          dardcor_CONFIG_CONTENT: JSON.stringify({
    ...
    const startRun = Effect.fn("dardcor.startRun")(function* (message: string, opts?: RunOpts) {
      const start = Date.now()
    ...
    const serve = Effect.fn("dardcor.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "dardcor server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `dardcor serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("dardcor.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const dardcor: dardcorCli = { run, startRun, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, dardcor })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `dardcor.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
      Layer.mergeAll(
        TestLLMServer.layer,
...
function expectExit(result: RunResult, expected: number, label = "dardcor") {
...
// every call site. Use this for any test that needs the dardcor CLI fixture.
//
...
// resources (e.g. `dardcor.serve`) without an extra `Effect.scoped` wrapper —
