...
import type { Event, dardcorClient } from "@dardcor-ai/sdk/v2"
...
type PermissionReplyParams = Parameters<dardcorClient["permission"]["reply"]>[0]
...
async function tempFile(name: string, content: string) {
  const dir = await mkdtemp(path.join(tmpdir(), "dardcor-acp-permission-"))
