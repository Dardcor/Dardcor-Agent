...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "dardcor.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "dardcor.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "dardcor.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "dardcor.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["dardcor.json", "dardcor.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "dardcor.json"),
        path.join(project, "dardcor.jsonc"),
      ])
    })
  })
