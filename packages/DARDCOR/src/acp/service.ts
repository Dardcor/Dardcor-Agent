...
import type { AssistantMessage, Message, dardcorClient, SessionMessageResponse } from "@dardcor-ai/sdk/v2"
...
export const AuthMethodID = "dardcor-login"
...
export class Service extends Context.Service<Service, Interface>()("@dardcor/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `dardcor auth login` in the terminal",
      name: "Login with dardcor",
      id: AuthMethodID,
    }
...
function makeDirectoryService(sdk: dardcorClient) {
...
function makeUsageService(sdk: dardcorClient) {
...
async function loadDirectorySnapshot(sdk: dardcorClient, directory: string) {
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, dardcor provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const dardcorProvider = providers[ProviderV2.ID.make("dardcor")]
  const dardcorModel = dardcorProvider ? Provider.sort(Object.values(dardcorProvider.models))[0] : undefined
  if (dardcorProvider && dardcorModel) return { providerID: dardcorProvider.id, modelID: dardcorModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "dardcor service failure", service })
}
