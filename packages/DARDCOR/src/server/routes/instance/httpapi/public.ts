...
import { dardcorHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for dardcor serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(dardcorHttpApi)
...
export const PublicApi = dardcorHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "dardcor",
    version: "1.0.0",
    description: "dardcor api",
