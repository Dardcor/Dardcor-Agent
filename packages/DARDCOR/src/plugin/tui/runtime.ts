...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@dardcor-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@dardcor-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@dardcor-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@dardcor-ai/tui/context/theme"
import { Global } from "@dardcor-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@dardcor-ai/core/util/flock"
import { Flag } from "@dardcor-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@dardcor-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@dardcor-ai/core/v1/config/plugin"
import { createCommandShim } from "@dardcor-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@dardcor-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".dardcor"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".dardcor", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".dardcor", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.dardcor_PURE ? [] : pluginOrigins
    if (Flag.dardcor_PURE && pluginOrigins.length) {
    }
