...
export const DEPRECATED_PLUGIN_PACKAGES = ["dardcor-openai-codex-auth", "dardcor-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, dardcorVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(dardcorVersion) || semver.major(dardcorVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.dardcor
  if (typeof range !== "string") return
  if (!semver.satisfies(dardcorVersion, range)) {
    throw new Error(`Plugin requires dardcor ${range} but running ${dardcorVersion}`)
  }
}
