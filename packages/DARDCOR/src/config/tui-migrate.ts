...
const TUI_SCHEMA_URL = "https://dardcor-code.vercel.app//tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from dardcor.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const dardcor = await dardcorFiles(input)
  for (const file of dardcor) {
...
async function dardcorFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "dardcor"),
    ...(await Filesystem.findUp(["dardcor.json", "dardcor.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "dardcor"))
  }
  if (Flag.dardcor_CONFIG) files.push(Flag.dardcor_CONFIG)
