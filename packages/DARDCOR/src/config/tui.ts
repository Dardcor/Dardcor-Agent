...
import { isRecord } from "@dardcor-ai/tui/util/record"
...
import { TuiKeybind } from "@dardcor-ai/tui/config/keybind"
...
import { TuiConfig } from "@dardcor-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@dardcor/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old dardcor.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.dardcor`
  // folders between cwd and home, and dardcor_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.dardcor_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit dardcor_TUI_CONFIG override, if set.
  if (Flag.dardcor_TUI_CONFIG) {
    const configFile = Flag.dardcor_TUI_CONFIG
  ...
  // 4. `.dardcor` directories (and dardcor_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".dardcor") || dir === Flag.dardcor_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".dardcor") && dir !== Flag.dardcor_CONFIG_DIR) continue
