...
import type { LLMClientShape } from "@dardcor-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("dardcor"))
    return { type: "unsupported", reason: "provider is not openai, dardcor, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @dardcor-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains dardcor-owned. The native runtime only adapts
      // the @dardcor-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
