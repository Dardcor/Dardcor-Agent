...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall dardcor and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall dardcor")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g dardcor-ai",
      pnpm: "pnpm uninstall -g dardcor-ai",
      bun: "bun remove -g dardcor-ai",
      yarn: "yarn global remove dardcor-ai",
      brew: "brew uninstall dardcor",
      choco: "choco uninstall dardcor",
      scoop: "scoop uninstall dardcor",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "dardcor", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using dardcor!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# dardcor") || content.includes(".dardcor/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".dardcor/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".dardcor/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".dardcor"))
    ) {
      continue
    }
