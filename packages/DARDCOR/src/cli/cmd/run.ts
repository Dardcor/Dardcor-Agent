import type { PermissionV1 } from "@dardcor-ai/core/v1/permission"
import { FSUtil } from "@dardcor-ai/core/fs-util"
// CLI entry point for `dardcor run` and `dardcor --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`dardcor --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`dardcor --mini --attach`): connects to a running
//      dardcor server and runs interactive mode against it.
//
...
import { createdardcorClient, type dardcorClient, type ToolPart } from "@dardcor-ai/sdk/v2"
import { FormatError, FormatUnknownError } from "../error"
import { INTERACTIVE_INPUT_ERROR, resolveInteractiveStdin } from "./run/runtime.stdin"

type ModelInput = Parameters<dardcorClient["session"]["prompt"]>[0]["model"]
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run dardcor with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running dardcor server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to dardcor_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to dardcor_SERVER_USERNAME or 'dardcor')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      async function session(sdk: dardcorClient): Promise<SessionInfo | undefined> {
        if (args.session) {
      ...
      async function share(sdk: dardcorClient, sessionID: string) {
      ...
      async function current(sdk: dardcorClient): Promise<string> {
        if (!args.attach) {
      ...
      async function attachAgent(sdk: dardcorClient) {
        if (!args.agent) return undefined
        const name = args.agent
      ...
      async function pickAgent(sdk: dardcorClient) {
        if (!args.agent) return undefined
        if (args.attach) {
      ...
      async function execute(sdk: dardcorClient) {
        ...
        async function loop(client: dardcorClient, events: Awaited<ReturnType<typeof sdk.event.subscribe>>) {
      ...
      const sdk = createdardcorClient({
        baseUrl: "http://dardcor.internal",
        fetch: fetchFn,
        directory,
      })
      await execute(sdk)
