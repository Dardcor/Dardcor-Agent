// Shared type vocabulary for the direct interactive mode (`dardcor --mini`).
//
...
import type { dardcorClient, PermissionRequest, QuestionRequest, ToolPart } from "@dardcor-ai/sdk/v2"
import type { TuiConfig } from "@dardcor-ai/tui/config"
...
type PromptModel = Parameters<dardcorClient["session"]["prompt"]>[0]["model"]
type PromptInput = Parameters<dardcorClient["session"]["prompt"]>[0]

export type RunPromptPart = NonNullable<PromptInput["parts"]>[number]

export type RunCommand = NonNullable<Awaited<ReturnType<dardcorClient["command"]["list"]>>["data"]>[number]

export type RunProvider = NonNullable<Awaited<ReturnType<dardcorClient["provider"]["list"]>>["data"]>["all"][number]
...
export type RunAgent = NonNullable<Awaited<ReturnType<dardcorClient["app"]["agents"]>>["data"]>[number]

type RunResourceMap = NonNullable<Awaited<ReturnType<dardcorClient["experimental"]["resource"]["list"]>>["data"]>
...
export type PermissionReply = Parameters<dardcorClient["permission"]["reply"]>[0]

export type QuestionReply = Parameters<dardcorClient["question"]["reply"]>[0]

export type QuestionReject = Parameters<dardcorClient["question"]["reject"]>[0]
