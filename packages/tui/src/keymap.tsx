...
export const dardcor_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const dardcor_MODE_KEY = "dardcor.mode"

export const DardcorKeymapProvider = KeymapProvider
export const usedardcorKeymap = useKeymap

export { useBindings, useKeymapSelector }

export type OpenTuiKeymap = ReturnType<typeof useKeymap>
type dardcorModeStack = ReturnType<typeof createdardcorModeStack>
...
const modeStacks = new WeakMap<OpenTuiKeymap, dardcorModeStack>()

function isVisiblePaletteCommand(command: Command) {
  return command.hidden !== true && command.name !== COMMAND_PALETTE_COMMAND
}

export function createdardcorModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(dardcor_MODE_KEY, dardcor_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(dardcor_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(dardcor_MODE_KEY, stack.at(-1)?.mode ?? dardcor_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? dardcor_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(dardcor_MODE_KEY, undefined)
...
export function usedardcorModeStack() {
  return getdardcorModeStack(usedardcorKeymap())
}

export function getdardcorModeStack(keymap: OpenTuiKeymap) {
  const value = modeStacks.get(keymap)
  if (!value) throw new Error("dardcor mode stack is not registered for this keymap")
  return value
}
...
export function registerdardcorKeymap(keymap: OpenTuiKeymap, renderer: CliRenderer, config: ResolvedKeymapConfig) {
  const modeStack = createdardcorModeStack(keymap)
