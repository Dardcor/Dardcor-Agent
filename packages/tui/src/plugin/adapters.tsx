import type { TuiDialogSelectOption, TuiPluginApi, TuiSlotProps } from "@dardcor-ai/plugin/tui"
...
import type { usedardcorKeymap } from "../keymap"
...
type Input = {
  ...
  keymap: ReturnType<typeof usedardcorKeymap>
...
export function createTuiApiAdapters(input: Input): Omit<TuiPluginApi, "lifecycle"> {
  return {
    ...
    mode: {
      current() {
        return Keymap.getdardcorModeStack(input.keymap).current()
      },
      push(mode) {
        return Keymap.getdardcorModeStack(input.keymap).push(mode)
      },
    },
    route: {
      register(list) {
