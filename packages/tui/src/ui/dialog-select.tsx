...
export function DialogSelect<T>(props: DialogSelectProps<T>) {
  ...
  return (
    <box gap={0} paddingBottom={1} flexGrow={1} backgroundColor={theme.backgroundPanel}>
      <box paddingLeft={2} paddingRight={2} paddingTop={1}>
        <box
          backgroundColor={tint(theme.backgroundElement, theme.primary, 0.18)}
          paddingLeft={2}
          paddingRight={2}
          paddingTop={1}
          paddingBottom={1}
                      ...
                      return (
                        ...
                        >
                          <box
                            ...
                            borderColor={active() ? theme.secondary : undefined}
                            backgroundColor={
                              active()
                                ? actionFocused()
                                  ? theme.backgroundElement
                                  : (option.bg ?? tint(theme.backgroundMenu, theme.primary, 0.35))
                                : current()
...
function Option(props: {
  ...
  const text = createMemo(() => {
    ...
    if (props.current) return theme.secondary
    return theme.text
  })
  ...
  return (
    <>
      <Show when={props.current && !props.gutter}>
        <text flexShrink={0} fg={theme.secondary} marginRight={0}>
          ▸
        </text>
      </Show>
