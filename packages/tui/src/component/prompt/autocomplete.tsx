...
import { useBindings, useCommandSlashes, usedardcorModeStack } from "../../keymap"
import { displayCharAt, mentionTriggerIndex } from "../../prompt/display"
import { SplitBorder } from "../../ui/border"
import type { FileSystemEntry } from "@dardcor-ai/sdk/v2"
...
export function Autocomplete(props: {
  ...
  return (
    <box
      ...
      position="absolute"
      top={position().y - height() - (store.visible === "/" ? 1 : 0)}
      left={position().x}
      ...
      <Show when={store.visible === "/"}>
        ...
        >
          <text fg={theme.primary}>
            ...
            <span style={{ fg: theme.textMuted }}>{search() ? ` · ${search()}` : " · search"}</span>
          </text>
          <text fg={theme.textMuted}>↑↓ select · enter run</text>
        </box>
      </Show>
      <scrollbox
