...
import { registerdardcorSpinner } from "../register-spinner"
...
import type { AssistantMessage, FilePart, UserMessage } from "@dardcor-ai/sdk/v2"
...
import { dardcor_BASE_MODE, useBindings, useCommandShortcut, useLeaderActive, usedardcorKeymap } from "../../keymap"
...
export function Prompt(props: PromptProps) {
  ...
  const clearShortcut = useCommandShortcut("prompt.clear")
  ...
  const promptCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        name: "workspace.set",
        category: "Session",
        enabled: Flag.dardcor_EXPERIMENTAL_WORKSPACES,
        slashName: "warp",
        run: () => {
          workspace.open()
        },
    ...
    <>
      <box ref={(r: BoxRenderable) => (anchor = r)} visible={props.visible !== false} width="100%">
        <box
          ...
          paddingTop={1}
          paddingBottom={1}
          backgroundColor={tint(theme.backgroundPanel, theme.primary, 0.08)}
        >
          ...
          <textarea
              ...
              placeholderColor={fadeColor(theme.textMuted, 0.78)}
              ...
              ref={(r: TextareaRenderable) => {
                ...
                setTimeout(() => {
                  // setTimeout is a workaround and needs to be addressed properly
                  if (!input || input.isDestroyed) return
                  input.cursorColor = theme.secondary
                }, 0)
              }}
              onMouseDown={(r: MouseEvent) => r.target?.focus()}
              focusedBackgroundColor={tint(theme.backgroundElement, theme.primary, 0.16)}
              cursorColor={props.disabled ? theme.backgroundElement : theme.secondary}
              syntaxStyle={syntax()}
            />
                  ...
                  {(agent) => (
                    <>
                      <text fg={fadeColor(highlight(), agentMetaAlpha())}>
                        {store.mode === "shell" ? "Shell" : agent().name === "build" ? "dardcor" : Locale.titlecase(agent().name)}
                      </text>
