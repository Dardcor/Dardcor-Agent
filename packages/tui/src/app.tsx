...
import { registerdardcorSpinner } from "./component/register-spinner"
...
import {
  ...
  dardcor_BASE_MODE,
  DardcorKeymapProvider,
  registerdardcorKeymap,
  useBindings,
  usedardcorKeymap,
} from "./keymap"
...
export const run = Effect.fn("Tui.run")(function* (input: TuiInput) {
  ...
  const result = yield* Effect.scoped(
    Effect.gen(function* () {
        ...
        Effect.tryPromise({
          try: () =>
            createCliRenderer({
              ...
              useMouse: !Flag.dardcor_DISABLE_MOUSE && input.config.mouse,
              consoleOptions: {
      ...
      const unguardCtrlC = win32InstallCtrlCGuard()
      yield* Effect.addFinalizer(() => Effect.sync(() => unguardCtrlC?.()))
      const uninstallSigint = Interrupt.installSigintHandler()
      yield* Effect.addFinalizer(() => Effect.sync(uninstallSigint))
      const keymap = createDefaultOpenTuiKeymap(renderer)
      yield* Effect.acquireRelease(
        Effect.sync(() => registerdardcorKeymap(keymap, renderer, input.config)),
                ...
                <ErrorBoundary fallback={(error, reset) => <ErrorComponent error={error} reset={reset} mode={mode} />}>
                  ...
                  >
                    ...
                    >
                      <TuiStartupProvider
                        value={{
                          initialRoute: process.env.dardcor_ROUTE ? JSON.parse(process.env.dardcor_ROUTE) : undefined,
                          skipInitialLoading: Boolean(process.env.dardcor_FAST_BOOT),
                        }}
                      >
                        <ClipboardProvider>
                          <DardcorKeymapProvider keymap={keymap}>
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  const offSelectionKeys = keymap.intercept(
    "key",
    ({ event }) => {
      if (!Flag.dardcor_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.dardcor_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("dardcor")
      return
    }
  ...
  onMount(() => {
    return Interrupt.set(() => {
      keymap.dispatchCommand("prompt.clear")
      return true
    })
  })
  onMount(() => {
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        category: "Workspace",
        hidden: !Flag.dardcor_EXPERIMENTAL_WORKSPACES,
      ...
      },
      {
        ...
        title: "Open docs",
        run: () => {
          open("https://dardcor-code.vercel.app//docs").catch(() => {})
          dialog.clear()
        },
        category: "System",
      },
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to dardcor v${result.data.version}. Please restart the application.`,
    )
  ...
  return (
    <box
      ...
      onMouseUp={
        !Flag.dardcor_EXPERIMENTAL_DISABLE_COPY_ON_SELECT
          ? () => Selection.copy(renderer, toast, clipboard)
          : undefined
      }
    >
      <Show when={Flag.dardcor_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
