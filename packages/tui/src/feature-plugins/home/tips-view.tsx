import type { TuiPluginApi } from "@dardcor-ai/plugin/tui"
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("dardcor.status"),
...
const TIPS: Tip[] = [
  ...
  (shortcuts) => press(shortcuts.agentCycle(), "to cycle between dardcor and Plan agents"),
  "Use {highlight}/undo{/highlight} to revert the last message and file changes",
  "Use {highlight}/redo{/highlight} to restore previously undone messages and file changes",
  "Run {highlight}/share{/highlight} to create a public dardcor.ai link",
  ...
  "Create {highlight}dardcor.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  "Place TUI settings in {highlight}~/.config/dardcor/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.dardcor/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.dardcor/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.dardcor/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.dardcor/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent dardcor from reading sensitive files",
  "Use {highlight}dardcor run{/highlight} for non-interactive scripting",
  "Use {highlight}dardcor --continue{/highlight} to resume the last session",
  "Use {highlight}dardcor run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}dardcor serve{/highlight} for headless API access to dardcor",
  "Use {highlight}dardcor run --attach{/highlight} to connect to a running server",
  "Run {highlight}dardcor upgrade{/highlight} to update to the latest version",
  "Run {highlight}dardcor auth list{/highlight} to see all configured providers",
  "Run {highlight}dardcor agent create{/highlight} for guided agent creation",
  "Use {highlight}/dardcor{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}dardcor github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/dardcor fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.dardcor/themes/{/highlight} directory",
  ...
  "Run {highlight}dardcor debug config{/highlight} to troubleshoot configuration",
  ...
  "Run {highlight}docker run -it --rm ghcr.io/anomalyco/dardcor{/highlight} in a container",
  "Use {highlight}/connect{/highlight} with darcor Zen for curated, tested models",
