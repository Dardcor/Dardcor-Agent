...
function View(props: { api: TuiPluginApi; sessionID: string }) {
  ...
  const has = createMemo(() =>
    props.api.state.provider.some(
      (item) => item.id !== "dardcor" || Object.values(item.models).some((model) => model.cost?.input !== 0),
    ),
  )
  ...
  return (
    <box gap={1}>
      <Show when={show()}>
        ...
        >
          ...
          <box flexGrow={1} gap={1}>
            ...
            <text fg={theme().textMuted}>dardcor includes free models so you can start immediately.</text>
      ...
      >
        ...
        <text fg={theme().textMuted}>
          <span style={{ fg: theme().primary }}>◆</span> <b>dardcor</b> <span>{props.api.app.version}</span>
        </text>
      </box>
    </box>
  )
