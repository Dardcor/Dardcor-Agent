...
import { dardcor_BASE_MODE, useBindings, useCommandShortcut, usedardcorKeymap } from "../../keymap"
...
const GO_UPSELL_PROVIDERS = new Set(["dardcor", "dardcor-go"])
    ...
    <LocationProvider location={location()}>
      ...
      >
        <box flexDirection="row" flexGrow={1} minHeight={0}>
          <box flexGrow={1} minHeight={0} paddingBottom={1} paddingLeft={3} paddingRight={3} gap={1}>
            <Show when={session()}>
              <scrollbox
                ref={(r) => (scroll = r)}
                viewportOptions={{
...
function AssistantMessage(props: { message: AssistantMessage; parts: Part[]; last: boolean }) {
  ...
  return (
    <box marginTop={2} flexShrink={0} gap={1}>
        ...
        <Match when={props.last || final() || props.message.error?.name === "MessageAbortedError"}>
          <box ref={(el: BoxRenderable) => alwaysSeparate.add(el)} paddingLeft={1}>
            <text marginTop={1} fg={theme.textMuted}>
              <span style={{ fg: theme.secondary }}>{model()}</span>
              <Show when={duration()}>
