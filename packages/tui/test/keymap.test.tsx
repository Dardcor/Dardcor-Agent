...
import { getdardcorModeStack, dardcor_BASE_MODE, DardcorKeymapProvider, registerdardcorKeymap } from "../src/keymap"
...
test("legacy page key aliases compile as page keys", async () => {
  ...
  function Harness() {
    ...
    const offKeymap = registerdardcorKeymap(keymap, renderer, config)
...
test("mode-less bindings stay active when dardcor mode changes", async () => {
  ...
  function Harness() {
    ...
    const popQuestion = getdardcorModeStack(keymap).push("question")
    counts.question = activeCounts()
    popQuestion()
    const popAutocomplete = getdardcorModeStack(keymap).push("autocomplete")
