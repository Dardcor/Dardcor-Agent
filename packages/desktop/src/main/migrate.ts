...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.dardcor.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `dardcor.settings.dat` is special: it maps to the `dardcor.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"dardcor.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // dardcor.settings.dat → the electron settings store ("dardcor.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "dardcor.global.dat").
  const storeName = filename === "dardcor.settings.dat" ? "dardcor.settings" : filename
