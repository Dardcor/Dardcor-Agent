...
import { expectdardcorVersion, pendingRestartAfterWslInstall, wslServerIdsToStartOnInitialize } from "./startup"
...
type WslServersControllerOptions = {
  ...
  resolvedardcor?: typeof resolveWsldardcor
...
export function createWslServersController(
  ...
  const setdardcorCheck = (distro: string, check: WsldardcorCheck) => {
    setState({
      dardcorChecks: {
        ...state.dardcorChecks,
        [distro]: check,
  ...
  const checkdardcor = async (distro: string, opts?: { signal?: AbortSignal }) => {
    const resolved = await (options?.resolvedardcor ?? resolveWsldardcor)(distro, opts)
    const version = resolved
      ? await (options?.readCommandVersion ?? readWslCommandVersion)(resolved, distro, opts)
      : null
    return dardcorCheck(distro, resolved, version, appVersion)
  }

  const refreshdardcorCheck = async (distro: string, opts?: { signal?: AbortSignal }) => {
    setdardcorCheck(distro, await checkdardcor(distro, opts))
  }
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const dardcorChecks = await Promise.all(
      unique
        .filter((distro) => distroProbeReady(state.distroProbes[distro]))
        .filter((distro) => !state.dardcorChecks[distro])
        .map(async (distro) => [distro, await checkdardcor(distro, opts)] as const),
    )
    if (dardcorChecks.length) {
      setState({ dardcorChecks: { ...state.dardcorChecks, ...Object.fromEntries(dardcorChecks) } })
    }
  }
  ...
  const refreshdardcorCheckBackground = (id: string, distro: string) => {
    void checkdardcor(distro)
      .then((check) => {
        if (!hasServer(id, distro)) return
        setdardcorCheck(distro, check)
      })
      .catch((error) => {
        const message = error instanceof Error ? error.message : String(error)
        logger?.error("wsl dardcor check failed", { id, distro, message })
      })
  }

  const refreshdardcorChecks = async () => {
    await Promise.all(
      state.servers.map((item) =>
        checkdardcor(item.config.distro)
          .then((check) => {
            if (!hasServer(item.config.id, item.config.distro)) return
            setdardcorCheck(item.config.distro, check)
          })
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl dardcor check failed", {
              id: item.config.id,
  ...
  const startServer = async (id: string) => {
    ...
    try {
      ...
      refreshdardcorCheckBackground(id, item.config.distro)
  ...
  return {
    ...
    async installdardcor(name: string) {
      await runJob({ kind: "install-dardcor", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWsldardcor(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "dardcor installation failed")
        }
        await refreshdardcorCheck(name, { signal: abort.signal })
        expectdardcorVersion(state.dardcorChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.dardcorChecks, distro) : {}),
      })
    },
...
function dardcorCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "dardcor is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "dardcor is installed but could not run",
    }
  }
  return {
    distro,
