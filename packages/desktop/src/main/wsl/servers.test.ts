...
let releasedardcorResolve: (() => void) | undefined
...
test("rejects an update that did not install the desktop version", () => {
  expect(() => expectdardcorVersion("1.16.2", "1.16.2")).not.toThrow()
  expect(() => expectdardcorVersion("1.14.35", "1.16.2")).toThrow(
    "dardcor update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating dardcor", () => {
  expect(
    wslServerIdToRestart(
      [
        {
...
test("clears cached distro probes when removing a WSL server", () => {
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.dardcor/bin/dardcor",
          version: "1.16.2",
  ...
  ).toEqual({ distroProbes: {}, dardcorChecks: {} })
})
...
test("ignores stale background dardcor checks after removing a WSL server", async () => {
  persistedServers = []
  releasedardcorResolve = undefined
  ...
  await waitFor(() => !!releasedardcorResolve)
  await controller.removeServer("wsl:Debian")
  releasedardcorResolve?.()
  await new Promise((resolve) => setTimeout(resolve, 0))

  expect(controller.getState().servers).toEqual([])
  expect(controller.getState().dardcorChecks).toEqual({})
})

test("ignores stale startup dardcor checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking dardcor", async () => {
  persistedServers = []
  const started: string[] = []
  const release = new Map<string, () => void>()
  const dardcor: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolvedardcor: async (distro) => {
      dardcor.push(distro)
      return "/home/me/.dardcor/bin/dardcor"
    },
  })
  ...
  expect(dardcor).toEqual([])
  release.get("Debian")?.()
  release.get("Ubuntu")?.()
  await task

  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(dardcor).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().dardcorChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check dardcor in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const dardcor: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(dardcor).toEqual(["Debian"])
  expect(Object.keys(controller.getState().dardcorChecks)).toEqual(["Debian"])
})
...
function testControllerOptions() {
  return {
    ...
    resolvedardcor: async () => {
      await new Promise<void>((resolve) => {
        releasedardcorResolve = resolve
      })
      return "/home/me/.dardcor/bin/dardcor"
    },
  }
