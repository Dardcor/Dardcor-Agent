import type { WslDistroProbe, WsldardcorCheck, WslServerItem } from "../../preload/types"
...
export function clearWslDistroState(
  ...
  dardcorChecks: Record<string, WsldardcorCheck>,
  distro: string,
) {
  const nextDistroProbes = { ...distroProbes }
  const nextdardcorChecks = { ...dardcorChecks }
  delete nextDistroProbes[distro]
  delete nextdardcorChecks[distro]
  return { distroProbes: nextDistroProbes, dardcorChecks: nextdardcorChecks }
}
