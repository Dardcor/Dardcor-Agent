...
export function registerWslIpcHandlers(controller: WslServersController) {
  ...
  ipcMain.handle("wsl-servers-install-dardcor", (_event: IpcMainInvokeEvent, name: string) =>
    controller.installdardcor(requireWslIpcString("distro", name)),
  )
...
function registerUnavailableWslIpcHandlers() {
  ...
  ipcMain.handle("wsl-servers-install-dardcor", unavailable)
