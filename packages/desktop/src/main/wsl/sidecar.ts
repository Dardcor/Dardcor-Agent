...
import { type WslCommandLine, resolveWsldardcor, shellEscape, wslArgs } from "./runtime"
...
export async function spawnWslSidecar(
  ...
  const dardcor = await resolveWsldardcor(distro)
  if (!dardcor) throw new Error(`dardcor is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "dardcor"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export dardcor_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export dardcor_CLIENT=desktop",
    `export dardcor_SERVER_USERNAME=${shellEscape(username)}`,
    `export dardcor_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(dardcor)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
