...
import type { UpdaterState } from "@dardcor-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@dardcor-ai/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.dardcor_CHANNEL === "prod" &&
...
const deepLinkEvent = "dardcor:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__dardcor__ ??= {}
  const pending = window.__dardcor__.deepLinks ?? []
  window.__dardcor__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `dardcor.desktop.window.${windowID}.last-active-url`
}
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://dardcor-code.vercel.app//favicon-96x96-v3.png",
      })
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("dardcor.global.dat").getItem("language")
