...
This is the living design and status document for `@dardcor-ai/codemode` and its existing V2 dardcor adapter.
...
`packages/dardcor/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
