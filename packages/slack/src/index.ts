...
import { createdardcor, type ToolPart } from "@dardcor-ai/sdk"
...
console.log("🚀 Starting dardcor server...")
const dardcor = await createdardcor({
  port: 0,
})
console.log("✅ dardcor server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
void (async () => {
  const events = await dardcor.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new dardcor session...")
    const { client, server } = dardcor
    ...
    console.log("✅ Created dardcor session:", createResult.data.id)
  ...
  console.log("📝 Sending to dardcor:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 dardcor response:", JSON.stringify(result, null, 2))
