...
As habilidades do agente permitem que o dardcor descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O dardcor pesquisa nesses locais:

- Configuração do projeto: `.dardcor/skills/<name>/SKILL.md`
- Configuração global: `~/.config/dardcor/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o dardcor sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.dardcor/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/dardcor/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.dardcor/skills/git-release/SKILL.md` assim:
...
O dardcor lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `dardcor.json`:
...
**Para agentes embutidos** (em `dardcor.json`):
