...
Com o dardcor, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o dardcor usa nosso próprio tema `dardcor`.
...
O dardcor vem com vários temas embutidos.
...
- Querem que o dardcor corresponda à aparência do seu terminal
...
O dardcor suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/dardcor/themes/*.json` ou `$XDG_CONFIG_HOME/dardcor/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.dardcor/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.dardcor/themes/*.json`
