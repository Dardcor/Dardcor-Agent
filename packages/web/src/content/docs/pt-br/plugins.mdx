---
title: Plugins
description: Escreva seus próprios plugins para estender o dardcor.
---

Plugins permitem que você estenda o dardcor conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do dardcor.
...
- `.dardcor/plugins/` - Plugins em nível de projeto
- `~/.config/dardcor/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/dardcor/node_modules/`.
...
1. Configuração global (`~/.config/dardcor/dardcor.json`)
2. Configuração do projeto (`dardcor.json`)
3. Diretório de plugins global (`~/.config/dardcor/plugins/`)
4. Diretório de plugins do projeto (`.dardcor/plugins/`)
...
O dardcor executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do dardcor para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o dardcor.
...
:::note
Se você estiver usando o aplicativo desktop dardcor, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o dardcor de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao dardcor:
...
O helper `tool` cria uma ferramenta personalizada que o dardcor pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o dardcor junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://dardcor-code.vercel.app//docs/sdk) para detalhes.
