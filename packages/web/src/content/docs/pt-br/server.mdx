---
title: Servidor
description: Interaja com o servidor dardcor via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `dardcor serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente dardcor pode usar.
...
```bash
dardcor serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `dardcor.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
dardcor serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `dardcor_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `dardcor`, ou defina `dardcor_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `dardcor serve` quanto ao `dardcor web`.

```bash
dardcor_SERVER_PASSWORD=your-password dardcor serve
```
...
Quando você executa `dardcor`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor dardcor para interagir com o dardcor programaticamente.
:::

Essa arquitetura permite que o dardcor suporte múltiplos clientes e permite que você interaja com o dardcor programaticamente.

Você pode executar `dardcor serve` para iniciar um servidor autônomo. Se você tiver o TUI do dardcor em execução, `dardcor serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do dardcor [IDE](/docs/ide).
...
O servidor dardcor expõe as seguintes APIs.
