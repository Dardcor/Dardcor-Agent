...
Existem dois tipos de agentes no dardcor; agentes primários e subagentes.
...
dardcor vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
dardcor vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

dardcor vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do dardcor, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `dardcor.json`:
...
- Global: `~/.config/dardcor/agents/`
- Por projeto: `.dardcor/agents/`
...
Se nenhuma temperatura for especificada, o dardcor usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do dardcor quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do dardcor usa o formato `provider/model-id`. Por exemplo, se você estiver usando [opencode Zen](/docs/zen), você usaria `dardcor/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="dardcor.json" {3-6,9-12}
{
  "$schema": "https://dardcor-code.vercel.app//config.json",
  "tools": {
    "write": true,
...
```json title="dardcor.json" {3-5,8-10}
{
  "$schema": "https://dardcor-code.vercel.app//config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `dardcor models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/anomalyco/dardcor).
:::
...
```markdown title="~/.config/dardcor/agents/docs-writer.md"
---
...
```markdown title="~/.config/dardcor/agents/security-auditor.md"
---
