---
title: المزوّدون
description: استخدام أي موفّر LLM في dardcor.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم dardcor ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات dardcor.
...
في `~/.local/share/dardcor/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات dardcor.
...
opencode Zen هي قائمة نماذج يوفّرها فريق dardcor وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع dardcor. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع opencode Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر dardcor، ثم انتقل إلى [dardcor.ai/auth](https://dardcor-code.vercel.app//auth).
...
يعمل مثل أي مزوّد آخر في dardcor واستخدامه اختياري بالكامل.

---

## opencode Go

opencode Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق dardcor، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع dardcor.

1. شغّل الأمر `/connect` في TUI، واختر `opencode Go`، ثم انتقل إلى [dardcor.ai/auth](https://opencode.ai).
...
لاستخدام Amazon Bedrock مع dardcor:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل dardcor:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `dardcor.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل dardcor غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين dardcor لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل dardcor بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات dardcor.
...
تخزين الرمز في مساحة تخزين مصادقة dardcor.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم dardcor نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر dardcor
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`dardcor.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [dardcor-gitlab-auth](https://www.npmjs.com/package/dardcor-gitlab-auth).

##### التهيئة

خصّص عبر `dardcor.json`:
...
لاستخدام اشتراك GitHub Copilot مع dardcor:
...
لاستخدام Google Vertex AI مع dardcor:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر dardcor، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [dardcor-helicone-session](https://github.com/H2Shami/dardcor-helicone-session) لتسجيل كل محادثة dardcor تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة dardcor مدرجة كجلسة مستقلة.
...
يمكنك ضبط dardcor لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط dardcor لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط dardcor لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع dardcor. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/dardcor) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع dardcor:
...
3. انسخ مفتاح API لاستخدامه في dardcor.
...
6. **مهم**: قبل استخدام نماذج السحابة في dardcor، يجب سحب معلومات النموذج محليا:
...
opencode Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق dardcor. [اعرف المزيد](/docs/zen).
...
3. أدخل مفتاح API الخاص بـ dardcor.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات dardcor.
...
5. يمكنك أيضا تخصيصها عبر إعدادات dardcor. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع dardcor:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات dardcor. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع dardcor. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `dardcor.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ dardcor بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `dardcor auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات dardcor وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات dardcor.
