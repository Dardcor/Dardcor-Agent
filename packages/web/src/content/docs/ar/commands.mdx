...
أنشئ الملف `.dardcor/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات dardcor أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ dardcor:
...
- على مستوى النظام: `~/.config/dardcor/commands/`
- لكل مشروع: `.dardcor/commands/`
...
يتضمن dardcor عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
