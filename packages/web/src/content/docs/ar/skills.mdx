...
تتيح مهارات الوكيل لـ dardcor اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث dardcor في هذه المواقع:

- إعدادات المشروع: `.dardcor/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/dardcor/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج dardcor صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.dardcor/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/dardcor/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.dardcor/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض dardcor المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `dardcor.json`:
...
**للوكلاء المدمجين** (في `dardcor.json`):
