...
مع dardcor يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم dardcor سمتنا `dardcor`.
...
يأتي dardcor مع عدة سمات مدمجة.
...
- يريدون أن يطابق dardcor مظهر terminal لديهم
...
يدعم dardcor نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/dardcor/themes/*.json` أو `$XDG_CONFIG_HOME/dardcor/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.dardcor/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.dardcor/themes/*.json`
