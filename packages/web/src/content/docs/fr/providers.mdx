---
title: Fournisseurs
description: Utiliser n’importe quel fournisseur LLM en dardcor.
---

import config from "../../../../config.mjs"
export const console = config.console

dardcor utilise [AI SDK](https://ai-sdk.dev/) et [Models.dev](https://models.dev) pour prendre en charge **75+ fournisseurs LLM** et prend en charge l'exécution de modèles locaux.

Pour ajouter un fournisseur, vous devez :

1. Ajoutez les clés API pour le fournisseur à l'aide de la commande `/connect`.
2. Configurez le fournisseur dans votre configuration dardcor.
...
en `~/.local/share/dardcor/auth.json`.

---

### Configuration

Vous pouvez personnaliser les fournisseurs via la section `provider` de votre dardcor
configuration.
...
opencode Zen est une liste de modèles fournis par l'équipe dardcor qui ont été
testé et vérifié pour fonctionner correctement avec dardcor. [En savoir plus](/docs/zen).

:::tip
Si vous êtes nouveau, nous vous recommandons de commencer par opencode Zen.
:::

1. Exécutez la commande `/connect` dans le TUI, sélectionnez dardcor et dirigez-vous vers [dardcor.ai/auth](https://dardcor-code.vercel.app//auth).
...
Il fonctionne comme n’importe quel autre fournisseur dans dardcor et son utilisation est totalement facultative.

---

## opencode Go

opencode Go est un plan d'abonnement à faible coût qui offre un accès fiable aux modèles de codage ouverts populaires fournis par l'équipe dardcor qui ont été
testé et vérifié pour fonctionner correctement avec dardcor.

1. Exécutez la commande `/connect` dans le TUI, sélectionnez `opencode Go` et rendez-vous sur [dardcor.ai/auth](https://opencode.ai).
...
Pour utiliser Amazon Bedrock avec dardcor :
...
Définissez l'une de ces variables d'environnement lors de l'exécution de dardcor :
...
Pour une configuration spécifique au projet ou persistante, utilisez `dardcor.json` :
...
:::info
L'utilisation de votre abonnement Claude Pro/Max dans dardcor n'est pas officiellement prise en charge par [Anthropic](https://anthropic.com).
:::
...
Vous pouvez configurer dardcor pour utiliser des modèles locaux via [Atomic Chat](https://atomic.chat), une application de bureau qui exécute des LLM locaux derrière un serveur API compatible OpenAI (point de terminaison par défaut `http://127.0.0.1:1337/v1`).
...
2. Accédez à [Azure AI Foundry](https://ai.azure.com/) et déployez un modèle.
   ...
   :::note
   Le nom du déploiement doit correspondre au nom du modèle pour que dardcor fonctionne correctement.
   :::
...
Vous pouvez également ajouter des modèles via votre configuration dardcor.
...
pour stocker le jeton dans le stockage d'authentification dardcor.
:::

##### GitLab auto-hébergé

:::note[note de conformité]
dardcor utilise un petit modèle pour certaines tâches d'IA telles que la génération du titre de la session.
Il est configuré pour utiliser gpt-5-nano par défaut, hébergé par Zen. Pour verrouiller dardcor
pour utiliser uniquement votre propre instance hébergée par GitLab, ajoutez ce qui suit à votre
Fichier `dardcor.json`. Il est également recommandé de désactiver le partage de session.
...
Plus de documentation sur la page d'accueil [dardcor-gitlab-auth](https://www.npmjs.com/package/dardcor-gitlab-auth).

##### Configuration

Personnalisez via `dardcor.json` :
...
Pour utiliser votre abonnement GitHub Copilot avec dardcor :
...
Pour utiliser Google Vertex AI avec dardcor :
...
Définissez-les lors de l'exécution de dardcor.
...
Si vous voyez une fonctionnalité ou un modèle d'Helicone qui n'est pas configuré automatiquement via dardcor, vous pouvez toujours le configurer vous-même.
...
La fonctionnalité [Sessions](https://docs.helicone.ai/features/sessions) d'Helicone vous permet de regrouper les requêtes LLM associées. Utilisez le plugin [dardcor-helicone-session](https://github.com/H2Shami/dardcor-helicone-session) pour enregistrer automatiquement chaque conversation dardcor en tant que session dans Helicone.
...
Le plugin injecte les en-têtes `Helicone-Session-Id` et `Helicone-Session-Name` dans vos requêtes. Sur la page Sessions d'Helicone, vous verrez chaque conversation dardcor répertoriée comme une session distincte.
...
Vous pouvez configurer dardcor pour utiliser des modèles locaux via l'utilitaire llama-server de [llama.cpp's](https://github.com/ggml-org/llama.cpp)
...
Vous pouvez configurer dardcor pour utiliser des modèles locaux via LM Studio.
...
Vous pouvez configurer dardcor pour utiliser des modèles locaux via Ollama.

:::tip
Ollama peut se configurer automatiquement pour dardcor. Voir les [documents d'intégration Ollama](https://docs.ollama.com/integrations/dardcor) pour plus de détails.
:::
...
Pour utiliser Ollama Cloud avec dardcor :
...
3. Copiez la clé API à utiliser dans dardcor.
...
6. **Important** : Avant d'utiliser des modèles cloud dans dardcor, vous devez extraire les informations du modèle localement :
...
opencode Zen est une liste de modèles testés et vérifiés fournie par l'équipe dardcor. [En savoir plus](/docs/zen).
...
3. Entrez votre clé dardcor API.
...
Vous pouvez également ajouter des modèles supplémentaires via votre configuration dardcor.
...
5. Vous pouvez également les personnaliser via votre configuration dardcor. Voici un exemple de spécification d'un fournisseur
...
Pour utiliser [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) avec dardcor :
...
Vous pouvez également personnaliser les modèles via votre configuration dardcor. Voici un exemple de spécification de l'ordre de routage du fournisseur.
...
:::tip
Vous pouvez utiliser n'importe quel fournisseur compatible OpenAI avec dardcor. La plupart des fournisseurs d'IA modernes proposent des API compatibles OpenAI.
:::
...
4. Créez ou mettez à jour votre fichier `dardcor.json` dans le répertoire de votre projet :
...
Les champs `limit` permettent à dardcor de comprendre la quantité de contexte qu'il vous reste. Les fournisseurs standard les extraient automatiquement de models.dev.
...
1. **Vérifiez la configuration de l'authentification** : exécutez `dardcor auth list` pour voir si les informations d'identification
...
2. Pour les fournisseurs personnalisés, vérifiez la configuration dardcor et :
   - Assurez-vous que l'ID du fournisseur utilisé dans la commande `/connect` correspond à l'ID de votre configuration dardcor.
