---
title: SDK
description: Client JS de type sécurisé pour le serveur dardcor.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Le SDK dardcor JS/TS fournit un client de type sécurisé pour interagir avec le serveur.
Utilisez-le pour créer des intégrations et contrôler dardcor par programme.
...
Créez une instance de dardcor :

```javascript
import { createdardcor } from "@dardcor-ai/sdk"

const { client } = await createdardcor()
```
...
Vous pouvez transmettre un objet de configuration pour personnaliser le comportement. L'instance récupère toujours votre `dardcor.json`, mais vous pouvez remplacer ou ajouter une configuration en ligne :
...
console.log(`Server running at ${dardcor.server.url}`)
...
Si vous disposez déjà d'une instance en cours d'exécution de dardcor, vous pouvez créer une instance client pour vous y connecter :
...
```typescript
import type { Session, Message, Part } from "@dardcor-ai/sdk"
```
...
const textResults = await client.find.text({
  query: { pattern: "function.*dardcor" },
})
