...
Créez `.dardcor/commands/test.md` :
...
Vous pouvez ajouter des commandes personnalisées via la configuration dardcor ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre dardcor [config](/docs/config) :
...
dardcor comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
