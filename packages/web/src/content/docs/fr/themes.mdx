...
Avec dardcor, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, dardcor utilise notre propre thème `dardcor`.
...
dardcor est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que dardcor corresponde à l'apparence de leur terminal
...
dardcor prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/dardcor/themes/*.json` ou `$XDG_CONFIG_HOME/dardcor/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.dardcor/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.dardcor/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/dardcor/themes
vim ~/.config/dardcor/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .dardcor/themes
vim .dardcor/themes/my-theme.json
```
...
{
  "$schema": "https://dardcor-code.vercel.app//theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
