---
title: Serveur
description: Interagissez avec le serveur dardcor via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `dardcor serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client dardcor peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `dardcor.local` |
...
Définissez `dardcor_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `dardcor`, ou définissez `dardcor_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `dardcor serve` et à `dardcor web`.
...
Lorsque vous exécutez `dardcor`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur dardcor pour interagir avec dardcor par programmation.
:::

Cette architecture permet à dardcor de prendre en charge plusieurs clients et vous permet d'interagir avec dardcor par programme.

Vous pouvez exécuter `dardcor serve` pour démarrer un serveur autonome. Si vous avez le
dardcor TUI en cours d'exécution, `dardcor serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins dardcor [IDE](/docs/ide).
...
Le serveur dardcor expose les API suivantes.
