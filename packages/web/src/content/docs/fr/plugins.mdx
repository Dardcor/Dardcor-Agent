---
title: Plugins
description: Écrivez vos propres plugins pour étendre dardcor.
---

Les plugins vous permettent d'étendre dardcor en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de dardcor.
...
- `.dardcor/plugins/` - Plugins au niveau du projet
- `~/.config/dardcor/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/dardcor/node_modules/`.
...
1. Configuration globale (`~/.config/dardcor/dardcor.json`)
2. Configuration du projet (`dardcor.json`)
3. Répertoire global des plugins (`~/.config/dardcor/plugins/`)
4. Répertoire des plugins du projet (`.dardcor/plugins/`)
...
dardcor exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK dardcor pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre dardcor.
...
:::note
Si vous utilisez l'application de bureau dardcor, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez dardcor de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à dardcor :
...
L'assistant `tool` crée un outil personnalisé que dardcor peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour dardcor aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://dardcor-code.vercel.app//docs/sdk) pour plus de détails.
