...
With dardcor you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, dardcor uses our own `dardcor` theme.
...
dardcor comes with several built-in themes.
...
- Want dardcor to match their terminal's appearance
...
dardcor supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/dardcor/themes/*.json` or `$XDG_CONFIG_HOME/dardcor/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.dardcor/themes/*.json`
4. **Current working directory** - Defined in `./.dardcor/themes/*.json`
