---
title: Server
description: Interact with dardcor server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `dardcor serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an dardcor client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `dardcor.local` |
...
Set `dardcor_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `dardcor`, or set `dardcor_SERVER_USERNAME` to override it. This applies to both `dardcor serve` and `dardcor web`.
...
When you run `dardcor` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the dardcor server to interact with dardcor programmatically.
:::

This architecture lets dardcor support multiple clients and allows you to interact with dardcor programmatically.

You can run `dardcor serve` to start a standalone server. If you have the
dardcor TUI running, `dardcor serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the dardcor [IDE](/docs/ide) plugins.
...
The dardcor server exposes the following APIs.
