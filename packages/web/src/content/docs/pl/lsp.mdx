---
title: Serwery LSP
description: dardcor integruje się z serwerami LSP.
---

dardcor może integrować się z serwerami Language Server Protocol (LSP), aby używać diagnostyki jako informacji zwrotnej dla agenta.

---

## Wbudowane

dardcor posiada kilka wbudowanych serwerów LSP dla następujących języków:
...
:::note
Możesz wyłączyć automatyczne pobieranie serwerów LSP, ustawiając zmienną środowiskową `dardcor_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Gdy LSP jest włączony i dardcor otwiera plik:
...
Możesz włączać i dostosowywać serwery LSP poprzez sekcję `lsp` w konfiguracji dardcor.
