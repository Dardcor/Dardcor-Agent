...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. dardcor posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w dardcor.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `dardcor_EXPERIMENTAL_LSP_TOOL=true` (lub `dardcor_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy dardcor lub gdy zmienna środowiskowa `dardcor_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu dardcor:
