...
Umiejętności agenta pozwalają dardcor odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
dardcor przeszukuje te lokalizacje:

- Project config: `.dardcor/skills/<name>/SKILL.md`
- Global config: `~/.config/dardcor/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu dardcor przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.dardcor/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/dardcor/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.dardcor/skills/git-release/SKILL.md` w ten sposób:
...
dardcor wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `dardcor.json`:
...
**Dla agentów wbudowanych** (w `dardcor.json`):
