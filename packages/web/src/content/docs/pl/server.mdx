---
title: Serwer
description: Interakcja z serwerem dardcor za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `dardcor serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient dardcor.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `dardcor.local` |
...
Ustaw `dardcor_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `dardcor` lub ustaw `dardcor_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `dardcor serve`, jak i `dardcor web`.
...
Kiedy uruchomisz `dardcor`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera dardcor do programowej interakcji z dardcor.
:::

Ta architektura umożliwia obsługę wielu klientów przez dardcor i programową interakcję z kodem otwartym.

Możesz uruchomić `dardcor serve`, aby uruchomić samodzielny serwer. Jeśli masz
dardcor TUI działa, `dardcor serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki dardcor [IDE](/docs/ide).
...
Serwer dardcor udostępnia następujące interfejsy API.
