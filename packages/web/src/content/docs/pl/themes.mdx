...
Dzięki dardcor możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie dardcor używa naszego własnego motywu `dardcor`.
...
dardcor ma kilka wbudowanych motywów.
...
- Chcą, aby dardcor pasował do wyglądu ich terminala
...
dardcor obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/dardcor/themes/*.json` lub `$XDG_CONFIG_HOME/dardcor/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.dardcor/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.dardcor/themes/*.json`
