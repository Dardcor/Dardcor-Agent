...
Możesz dodać dodatkowe informacje do dardcor za pomocą _Model Context Protocol_, w skrócie MCP. dardcor obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [dardcor Config](https://dardcor-code.vercel.app//docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/dardcor`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
dardcor automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, dardcor:
...
Jeśli serwer wymaga uwierzytelnienia, dardcor poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `dardcor mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji dardcor bezpieczne przechowa tokeny w `~/.local/share/dardcor/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w dardcor, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie dardcor, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć dardcor z kontem Sentry.
