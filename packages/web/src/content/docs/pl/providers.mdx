---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w dardcor.
---

import config from "../../../../config.mjs"
export const console = config.console

dardcor używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji dardcor.
...
w `~/.local/share/dardcor/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim dardcor
config.
...
opencode Zen to lista modeli dostarczonych przez zespół dardcor, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z dardcor. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od opencode Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `opencode Zen` i przejdź do [dardcor.ai/auth](https://opencode.ai).
...
Działa jak każdy inny dostawca w dardcor i jest całkowicie opcjonalny w użyciu.

---

## opencode Go

opencode Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół dardcor, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z dardcor.

1. Uruchom polecenie `/connect` w TUI, wybierz `opencode Go` i przejdź do [dardcor.ai/auth](https://opencode.ai).
...
Aby używać Amazon Bedrock z dardcor:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania dardcor:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `dardcor.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w dardcor nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować dardcor do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod dardcor działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji dardcor.
...
dardcor używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować dardcor
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`dardcor.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [dardcor-gitlab-auth](https://www.npmjs.com/package/dardcor-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem dardcor:
...
Aby używać Google Vertex AI z dardcor:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania dardcor.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą dardcor, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [dardcor-helicone-session](https://github.com/H2Shami/dardcor-helicone-session), aby automatycznie rejestrować każdą konwersację dardcor jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja dardcor będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować dardcor tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować dardcor tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować dardcor tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem dardcor. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/dardcor).
:::
...
Aby korzystać z Ollama Cloud z dardcor:
...
3. Skopiuj klucz API do użycia w dardcor.
...
6. **Ważne**: Przed użyciem modeli chmurowych w dardcor musisz pobrać informacje o modelu lokalnie:
...
opencode Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół dardcor. [Dowiedz się więcej](/docs/zen).
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji dardcor.
...
5. Możesz je także dostosować za pomocą konfiguracji dardcor. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z dardcor:
...
Możesz także dostosować modele za pomocą konfiguracji dardcor. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z dardcor. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `dardcor.json` w katalogu projektu:
...
Pola `limit` pozwalają dardcor zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `dardcor auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację dardcor i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji dardcor.
