---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć dardcor.
---

Wtyczki umożliwiające wyłączenie dardcor poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania dardcor.
...
- `.dardcor/plugins/` - Wtyczki na poziomie projektu
- `~/.config/dardcor/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/dardcor/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/dardcor/dardcor.json`)
2. Konfiguracja projektu (`dardcor.json`)
3. Globalny katalog wtyczek (`~/.config/dardcor/plugins/`)
4. Katalog wtyczek projektu (`.dardcor/plugins/`)
...
dardcor uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z dardcor do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do dardcor.
...
:::note
Jeśli korzystasz z aplikacji komputerowej dardcor, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj dardcor czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do dardcor:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod dardcor. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://dardcor-code.vercel.app//docs/sdk).
