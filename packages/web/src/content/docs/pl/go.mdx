...
Go działa jak każdy inny dostawca w dardcor. Subskrybujesz opencode Go i
...
opencode Go działa jak każdy inny dostawca w dardcor.
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji dardcor
używa formatu `dardcor-go/<model-id>`. Na przykład dla Kimi K2.7 Code należy użyć
`dardcor-go/kimi-k2.7-code` w swojej konfiguracji.
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z dardcor.
