...
Aby debugować problemy z dardcor, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\dardcor\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `dardcor --log-level DEBUG`.

---

## Przechowywanie danych

dardcor przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/dardcor/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\dardcor`
...
dardcor Desktop uruchamia w tle lokalny serwer dardcor (sidecar `dardcor-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `dardcor` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/dardcor/dardcor.jsonc` (lub `~/.config/dardcor/dardcor.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/dardcor/dardcor.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\dardcor\dardcor.jsonc`
...
dardcor może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/dardcor/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\dardcor\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby dardcor mógł ją odbudować.

1. Całkowicie zamknij dardcor Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/dardcor`
- **Linux**: usuń `~/.cache/dardcor` (lub uruchom `rm -rf ~/.cache/dardcor`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\dardcor`

3. Uruchom ponownie dardcor Desktop.

---

### Napraw problemy z połączeniem z serwerem

dardcor Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `dardcor.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `dardcor_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `dardcor_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows dardcor Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami dardcor.

---

### Brak powiadomień

dardcor Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla dardcor w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie dardcor Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji dardcor Desktop):

- `dardcor.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `dardcor.global.dat` i `dardcor.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `dardcor upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `dardcor models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\dardcor`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. dardcor dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\dardcor`

2. Uruchom ponownie kod dardcor, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to dardcor do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
dardcor wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
