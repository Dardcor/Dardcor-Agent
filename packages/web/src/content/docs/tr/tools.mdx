...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. dardcor yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
dardcor içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `dardcor_EXPERIMENTAL_LSP_TOOL=true` (veya `dardcor_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca dardcor provider kullanılırken veya `dardcor_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

dardcor başlatırken etkinleştirmek için:
