---
title: Sağlayıcılar
description: dardcor'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

dardcor, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı dardcor yapılandırmanızda yapılandırın.
...
`~/.local/share/dardcor/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları dardcor'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
opencode Zen, dardcor ekibi tarafından sağlanan modellerin bir listesidir.
dardcor ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz opencode Zen ile başlamanızı öneririz.
:::

1. TUI'de `/connect` komutunu çalıştırın, dardcor'u seçin ve [dardcor.ai/auth](https://dardcor-code.vercel.app//auth)'ye gidin.
...
dardcor'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## opencode Go

opencode Go, dardcor ile iyi çalıştığı test edilmiş ve doğrulanmış, dardcor ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `opencode Go`'yu seçin ve [dardcor.ai/auth](https://opencode.ai) adresine gidin.
...
Amazon Bedrock'u dardcor ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   dardcor'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `dardcor.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi dardcor'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
dardcor'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   dardcor'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   dardcor yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
dardcor, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. dardcor'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`dardcor.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [dardcor-gitlab-auth](https://www.npmjs.com/package/dardcor-gitlab-auth) ana sayfasında.

##### Yapılandırma

`dardcor.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi dardcor'la kullanmak için:
...
Google Vertex AI'yi dardcor ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları dardcor çalıştırılırken ayarlayın.
...
Helicone'un dardcor aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her dardcor görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [dardcor-helicone-session](https://github.com/H2Shami/dardcor-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her dardcor görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
dardcor'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla dardcor'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla dardcor'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini dardcor için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/dardcor)'a bakın.
:::
...
Ollama Cloud'u dardcor ile kullanmak için:
...
3. dardcor'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: dardcor'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
opencode Zen, dardcor ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).
...
3. dardcor API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   dardcor yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca dardcor yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi dardcor ile kullanmak için:
...
Modelleri dardcor yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı dardcor ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `dardcor.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları dardcor'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `dardcor auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için dardcor yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, dardcor yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
