...
Go, dardcor'daki diğer sağlayıcılar gibi çalışır. opencode Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve dardcor'u kullanmak için bunu kullanmanıza gerek yoktur.
...
opencode Go, dardcor'daki diğer sağlayıcılar gibi çalışır.
...
dardcor yapılandırmanızdaki [model id](/docs/config/#models) formatı `dardcor-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `dardcor-go/kimi-k2.7-code` kullanmalısınız.
...
4. dardcor ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
