---
title: Eklentiler
description: dardcor'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek dardcor'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya dardcor'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.dardcor/plugins/` - Proje düzeyinde eklentiler
- `~/.config/dardcor/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/dardcor/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/dardcor/dardcor.json`)
2. Project config (`dardcor.json`)
3. Global eklenti dizini (`~/.config/dardcor/plugins/`)
4. Proje eklenti dizini (`.dardcor/plugins/`)
...
dardcor bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir dardcor SDK istemcisi.
...
dardcor'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
dardcor masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
dardcor'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca dardcor'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, dardcor'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra dardcor'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://dardcor-code.vercel.app//docs/sdk) bakın.
