...
description: dardcor, LSP sunucularınızla bütünleşir.
---

dardcor, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

dardcor, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`dardcor_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve dardcor bir dosyayı açtığında:
...
LSP sunucularını, dardcor yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
