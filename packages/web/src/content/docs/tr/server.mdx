---
title: Sunucu
description: dardcor sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`dardcor serve` komutu, dardcor istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `dardcor.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `dardcor_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `dardcor` değeridir; değiştirmek isterseniz `dardcor_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `dardcor serve` hem de `dardcor web` için geçerlidir.
...
`dardcor` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
dardcor ile programatik etkileşim için dardcor sunucusunu kullanın.
:::

Bu mimari, dardcor'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`dardcor serve` ile bağımsız bir sunucu başlatabilirsiniz. dardcor TUI açıksa, `dardcor serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum dardcor [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
dardcor sunucusu aşağıdaki API'leri sunar.
