...
dardcor ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\dardcor\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `dardcor --log-level DEBUG`.

---

## Depolama

dardcor, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/dardcor/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\dardcor` yapistirin
...
dardcor Desktop arka planda yerel bir dardcor sunucusu (`dardcor-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `dardcor` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/dardcor/dardcor.jsonc` (veya `~/.config/dardcor/dardcor.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/dardcor/dardcor.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\dardcor\dardcor.jsonc` yapistirin
...
dardcor diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/dardcor/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\dardcor\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), dardcor'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. dardcor Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/dardcor`
- **Linux**: `~/.cache/dardcor` dizinini silin (veya `rm -rf ~/.cache/dardcor` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\dardcor` yapistirin

3. dardcor Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

dardcor Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`dardcor.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `dardcor_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `dardcor_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta dardcor Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, dardcor ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

dardcor Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda dardcor icin bildirimler etkinse
...
1. dardcor Desktop'u kapatin
2. Su dosyalari bulun ve silin (dardcor Desktop uygulama veri dizininde yer alirlar):

- `dardcor.settings.dat` (desktop varsayilan sunucu URL'si)
- `dardcor.global.dat` ve `dardcor.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
dardcor ile ilgili bir sorun yasiyorsaniz:
...
3. `dardcor upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `dardcor models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\dardcor`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. dardcor, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\dardcor`

2. En guncel provider paketlerini yeniden kurmak icin dardcor'u yeniden baslatin

Bu, dardcor'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
dardcor, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
