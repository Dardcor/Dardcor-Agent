...
Ajan becerileri, dardcor'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
dardcor su konumlari tarar:

- Proje konfigurasyonu: `.dardcor/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/dardcor/skills/<name>/SKILL.md`
...
Proje yerel yollarinda dardcor, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.dardcor/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/dardcor/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.dardcor/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
dardcor, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`dardcor.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`dardcor.json` icinde):
