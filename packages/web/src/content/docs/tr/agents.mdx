...
dardcor'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
dardcor, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
dardcor, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

dardcor iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `dardcor.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse dardcor modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global dardcor yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
dardcor yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [opencode Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `dardcor/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `dardcor models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/anomalyco/dardcor).
:::
