...
dardcor ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak dardcor kendi `dardcor` temasını kullanır.
...
dardcor birden fazla yerleşik temayla gelir.
...
- dardcor'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
dardcor, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/dardcor/themes/*.json` veya `$XDG_CONFIG_HOME/dardcor/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.dardcor/themes/*.json`
4. **Geçerli çalışma dizini** - `./.dardcor/themes/*.json`
