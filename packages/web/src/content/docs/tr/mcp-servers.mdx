...
_Model Bağlam Protokolü_ veya MCP kullanarak dardcor'a harici araçlar ekleyebilirsiniz. dardcor hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [dardcor Config](https://dardcor-code.vercel.app//docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/dardcor` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
dardcor, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde dardcor şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa dardcor, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `dardcor mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından dardcor, belirteçleri `~/.local/share/dardcor/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra dardcor'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi dardcor yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve dardcor'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
