...
Создайте `.dardcor/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию dardcor или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем dardcor [config](/docs/config):
...
- Глобальный: `~/.config/dardcor/commands/`
- Для каждого проекта: `.dardcor/commands/`
...
dardcor включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
