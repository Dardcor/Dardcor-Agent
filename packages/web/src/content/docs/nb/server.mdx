---
title: Tjener
description: Samhandle med dardcor-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `dardcor serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en dardcor-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `dardcor.local` |
...
Sett `dardcor_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `dardcor` som standard, eller sett `dardcor_SERVER_USERNAME` for å overstyre det. Dette gjelder både `dardcor serve` og `dardcor web`.
...
Når du kjører `dardcor` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk dardcor-serveren til å samhandle med dardcor programmatisk.
:::

Denne arkitekturen lar dardcor støtte flere klienter og lar deg samhandle med dardcor programmatisk.

Du kan kjøre `dardcor serve` for å starte en frittstående server. Hvis du har
dardcor TUI kjører, vil `dardcor serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av dardcor [IDE](/docs/ide) plugins.
...
dardcor-serveren viser følgende APIer.
