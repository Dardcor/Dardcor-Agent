...
description: Skriv dine egne programtillegg for å utvide dardcor.
---

Plugins lar deg utvide dardcor ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til dardcor.
...
- `.dardcor/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/dardcor/node_modules/`.
...
1. Global konfigurasjon (`~/.config/dardcor/dardcor.json`)
2. Prosjektkonfigurasjon (`dardcor.json`)
3. Global plugin-katalog (`~/.config/dardcor/plugins/`)
4. Prosjektpluginkatalog (`.dardcor/plugins/`)
...
dardcor kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En dardcor SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide dardcor.
...
:::note
Hvis du bruker dardcor-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre dardcor fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til dardcor:
...
`tool`-hjelperen lager et tilpasset verktøy som dardcor kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for dardcor sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://dardcor-code.vercel.app//docs/sdk) for detaljer.
