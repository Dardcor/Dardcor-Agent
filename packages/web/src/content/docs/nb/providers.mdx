---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i dardcor.
---

import config from "../../../../config.mjs"
export const console = config.console

dardcor bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i dardcor-konfigurasjonen.
...
i `~/.local/share/dardcor/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i dardcor
konfig.
...
opencode Zen er en liste over modeller levert av dardcor-teamet som har vært
testet og verifisert for å fungere godt med dardcor. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med opencode Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `opencode Zen` og gå til [dardcor.ai/auth](https://opencode.ai).
...
Det fungerer som alle andre leverandører i dardcor og er helt valgfritt å bruke.

---

## opencode Go

opencode Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av dardcor-teamet som har vært
testet og verifisert for å fungere godt med dardcor.

1. Kjør kommandoen `/connect` i TUI, velg `opencode Go`, og gå til [dardcor.ai/auth](https://opencode.ai).
...
Slik bruker du Amazon Bedrock med dardcor:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører dardcor:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `dardcor.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i dardcor støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere dardcor til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at dardcor skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom dardcor-konfigurasjonen.
...
for å lagre token i dardcor auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
dardcor bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse dardcor
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`dardcor.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [dardcor-gitlab-auth](https://www.npmjs.com/package/dardcor-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `dardcor.json`:
...
Slik bruker du GitHub Copilot-abonnementet med dardcor:
...
Slik bruker du Google Vertex AI med dardcor:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører dardcor.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom dardcor, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [dardcor-helicone-session](https://github.com/H2Shami/dardcor-helicone-session) plugin for automatisk å logge hver dardcor-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver dardcor-samtale oppført som en separat økt.
...
Du kan konfigurere dardcor for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere dardcor for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere dardcor for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for dardcor. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/dardcor) for detaljer.
:::
...
Slik bruker du Ollama Cloud med dardcor:
...
3. Kopier API-nøkkelen for bruk i dardcor.
...
6. **Viktig**: Før du bruker skymodeller i dardcor, må du hente modellinformasjonen lokalt:
...
opencode Zen er en liste over testede og verifiserte modeller levert av dardcor-teamet. [Finn ut mer](/docs/zen).
...
3. Skriv inn dardcor API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom dardcor-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom dardcor-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med dardcor:
...
Du kan også tilpasse modeller gjennom dardcor-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med dardcor. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `dardcor.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar dardcor forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `dardcor auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk dardcor-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i dardcor-konfigurasjonen.
