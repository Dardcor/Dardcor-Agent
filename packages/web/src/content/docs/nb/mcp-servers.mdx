...
Du kan legge til eksterne verktøy i dardcor ved å bruke _Model Context Protocol_ eller MCP. dardcor støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [dardcor Config](https://dardcor-code.vercel.app//docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="dardcor.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/dardcor`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
dardcor håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil dardcor:
...
Hvis serveren krever autentisering, vil dardcor be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `dardcor mcp auth <server-name>`.
...
```bash
dardcor mcp auth my-oauth-server
```
...
```bash
dardcor mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil dardcor lagre tokens sikkert i `~/.local/share/dardcor/mcp-auth.json`.
...
dardcor mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i dardcor, sammen med innebygde verktøy. Så du kan administrere dem gjennom dardcor-konfigurasjonen som alle andre verktøy.
...
```json title="dardcor.json" {11, 14-18}
{
  "$schema": "https://dardcor-code.vercel.app//config.json",
  "mcp": {
    "my-mcp": {
...
```bash
dardcor mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble dardcor til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver dardcor-samtale oppført som en separat økt.
