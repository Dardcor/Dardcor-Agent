...
Det er to typer agenter i dardcor; primære agenter og underagenter.
...
dardcor kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
dardcor kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

dardcor kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `dardcor.json`:
...
Hvis ingen temperatur er spesifisert, bruker dardcor modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale dardcor-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i dardcor-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [opencode Zen](/docs/zen), vil du bruke `dardcor/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `dardcor models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/anomalyco/dardcor).
:::
