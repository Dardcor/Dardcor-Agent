...
Go works like any other provider in dardcor. You subscribe to opencode Go and
...
opencode Go works like any other provider in dardcor.
...
uses the format `dardcor-go/<model-id>`. For example, for Kimi K2.7 Code, you would
use `dardcor-go/kimi-k2.7-code` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with dardcor as well.
