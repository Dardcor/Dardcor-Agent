...
Go 的工作方式与 dardcor 中的任何其他提供商（provider）一样。订阅 opencode Go 后你将获得 API 密钥。它是 **完全可选** 的，并非使用 dardcor 所必需的条件。
...
:::tip
我们测试了一组经过精选且与 dardcor 配合良好的模型和提供商。
:::
...
opencode Go 的工作方式与 dardcor 中的其他提供商一样。
...
你 dardcor 配置中的 [模型 ID](/docs/config/#models) 使用 `dardcor-go/<model-id>` 格式。例如，对于 Kimi K2.7 Code，你将在配置中使用 `dardcor-go/kimi-k2.7-code`。
...
4. **无锁定（no lock-in）**，允许你与 dardcor 一起使用任何其他提供商。
