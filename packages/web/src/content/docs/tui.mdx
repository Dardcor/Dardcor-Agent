---
title: TUI
description: Using the dardcor terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

dardcor provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running dardcor starts the TUI for the current directory.
...
When using the dardcor TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to dardcor. Allows you to select from available providers and add their API keys.
...
This is separate from `dardcor.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long dardcor waits after the leader key. Defaults to `2000`.
...
Use `dardcor_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `dardcor.default`.
