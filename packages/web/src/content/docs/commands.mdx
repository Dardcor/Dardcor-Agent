...
Create `.dardcor/commands/test.md`:
...
You can add custom commands through the dardcor config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your dardcor [config](/docs/config):
...
dardcor includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
