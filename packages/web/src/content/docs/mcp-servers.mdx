...
You can add external tools to dardcor using the _Model Context Protocol_, or MCP. dardcor supports both local and remote servers.
...
You can define MCP servers in your [dardcor Config](https://dardcor-code.vercel.app//docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/dardcor` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
dardcor automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, dardcor will:
...
If the server requires authentication, dardcor will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `dardcor mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, dardcor will store the tokens securely in `~/.local/share/dardcor/mcp-auth.json`.
...
Your MCPs are available as tools in dardcor, alongside built-in tools. So you can manage them through the dardcor config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect dardcor to your Sentry account.
