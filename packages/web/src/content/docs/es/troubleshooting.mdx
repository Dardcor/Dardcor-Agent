...
Para depurar problemas con dardcor, comience verificando los registros y los datos locales que almacena en el disco.
...
- **macOS/Linux**: `~/.local/share/dardcor/log/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\dardcor\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `dardcor --log-level DEBUG`.

---

## Almacenamiento

dardcor almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/dardcor/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\dardcor`
...
dardcor Desktop ejecuta un servidor dardcor local (el sidecar `dardcor-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `dardcor` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/dardcor/dardcor.jsonc` (o `~/.config/dardcor/dardcor.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/dardcor/dardcor.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\dardcor\dardcor.jsonc`
...
dardcor también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/dardcor/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\dardcor\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.dardcor/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que dardcor pueda reconstruirlo.

1. Salga de dardcor Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/dardcor`
- **Linux**: eliminar `~/.cache/dardcor` (o ejecutar `rm -rf ~/.cache/dardcor`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\dardcor`

3. Reinicie el escritorio dardcor.

---

### Solucionar problemas de conexión del servidor

dardcor Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `dardcor.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `dardcor_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `dardcor_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio dardcor requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de dardcor.

---

### Notificaciones que no se muestran

dardcor Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para dardcor en la configuración de su sistema operativo, y
...
1. Salga del escritorio dardcor.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio dardcor):

- `dardcor.settings.dat` (URL del servidor predeterminado de escritorio)
- `dardcor.global.dat` y `dardcor.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con dardcor:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/anomalyco/dardcor/issues**](https://github.com/anomalyco/dardcor/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**dardcor.ai/discord**](https://dardcor-code.vercel.app//discord)
...
3. Asegúrese de tener la última versión con `dardcor upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `dardcor models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/dardcor
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\dardcor`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. dardcor instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\dardcor`

2. Reinicie dardcor para reinstalar los paquetes del proveedor más recientes.

Esto obligará a dardcor a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
dardcor detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
