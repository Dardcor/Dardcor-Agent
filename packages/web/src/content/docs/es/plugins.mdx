---
title: Complementos
description: Escriba sus propios complementos para extender dardcor.
---

Los complementos le permiten extender dardcor al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de dardcor.
...
- `.dardcor/plugins/` - Complementos a nivel de proyecto
- `~/.config/dardcor/plugins/` - Complementos globales
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/dardcor/node_modules/`.
...
1. Configuración global (`~/.config/dardcor/dardcor.json`)
2. Configuración del proyecto (`dardcor.json`)
3. Directorio global de complementos (`~/.config/dardcor/plugins/`)
4. Directorio de complementos del proyecto (`.dardcor/plugins/`)
...
dardcor ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK dardcor para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar dardcor.
...
:::note
Si está utilizando la aplicación de escritorio dardcor, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que dardcor lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a dardcor:
...
El ayudante `tool` crea una herramienta personalizada a la que dardcor puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para dardcor junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://dardcor-code.vercel.app//docs/sdk) para obtener más detalles.
