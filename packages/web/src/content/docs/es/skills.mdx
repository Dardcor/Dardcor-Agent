...
Las habilidades del agente permiten a dardcor descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
dardcor busca estas ubicaciones:

- Configuración del proyecto: `.dardcor/skills/<name>/SKILL.md`
- Configuración global: `~/.config/dardcor/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, dardcor sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.dardcor/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/dardcor/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.dardcor/skills/git-release/SKILL.md` así:
...
dardcor enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `dardcor.json`:
...
**Para agentes integrados** (en `dardcor.json`):
