...
Las herramientas permiten que LLM realice acciones en su código base. dardcor viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en dardcor.
...
:::note
Esta herramienta solo está disponible cuando `dardcor_EXPERIMENTAL_LSP_TOOL=true` (o `dardcor_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor dardcor o cuando la variable de entorno `dardcor_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar dardcor:

```bash
dardcor_ENABLE_EXA=1 dardcor
```
