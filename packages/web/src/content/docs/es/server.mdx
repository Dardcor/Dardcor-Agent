---
title: Servidor
description: Interactuar con el servidor dardcor a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `dardcor serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente dardcor puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `dardcor.local` |
...
Configure `dardcor_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `dardcor`, o configure `dardcor_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `dardcor serve` como a `dardcor web`.
...
Cuando ejecuta `dardcor`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor dardcor para interactuar con dardcor mediante programación.
:::

Esta arquitectura permite que dardcor admita múltiples clientes y le permite interactuar con dardcor mediante programación.

Puede ejecutar `dardcor serve` para iniciar un servidor independiente. Si tienes el
dardcor TUI ejecutándose, `dardcor serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos dardcor [IDE](/docs/ide).
...
El servidor dardcor expone las siguientes API.
