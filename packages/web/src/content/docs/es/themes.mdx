...
Con dardcor puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, dardcor usa nuestro propio tema `dardcor`.
...
dardcor viene con varios temas integrados.
...
- Quiere que dardcor coincida con la apariencia de su terminal
...
dardcor admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/dardcor/themes/*.json` o `$XDG_CONFIG_HOME/dardcor/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.dardcor/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.dardcor/themes/*.json`
