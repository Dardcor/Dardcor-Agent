---
title: Proveedores
description: Usando cualquier proveedor LLM en dardcor.
---

import config from "../../../../config.mjs"
export const console = config.console

dardcor usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración dardcor.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu dardcor
configuración.
...
opencode Zen es una lista de modelos proporcionados por el equipo dardcor que han sido
probado y verificado para funcionar bien con dardcor. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con opencode Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione dardcor y diríjase a [dardcor.ai/auth](https://dardcor-code.vercel.app//auth).
...
Funciona como cualquier otro proveedor en dardcor y su uso es completamente opcional.

---

## opencode Go

opencode Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de dardcor que han sido
probado y verificado para funcionar bien con dardcor.

1. Ejecute el comando `/connect` en TUI, seleccione `opencode Go` y diríjase a [dardcor.ai/auth](https://opencode.ai).
...
Para usar Amazon Bedrock con dardcor:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta dardcor:
   ...
   Para una configuración persistente o específica del proyecto, utilice `dardcor.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en dardcor no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar dardcor para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que dardcor funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración dardcor.
...
para almacenar el token en el almacenamiento de autenticación dardcor.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
dardcor utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear dardcor
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `dardcor.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [dardcor-gitlab-auth](https://www.npmjs.com/package/dardcor-gitlab-auth).

##### Configuración

Personalizar a través de `dardcor.json`:
...
Para utilizar su suscripción GitHub Copilot con dardcor:
...
Para utilizar Google Vertex AI con dardcor:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta dardcor.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de dardcor, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [dardcor-helicone-session](https://github.com/H2Shami/dardcor-helicone-session) para registrar automáticamente cada conversación dardcor como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación dardcor enumerada como una sesión separada.
...
Puede configurar dardcor para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar dardcor para usar modelos locales a través de LM Studio.
...
Puedes configurar dardcor para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para dardcor. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/dardcor) para obtener más detalles.
:::
...
Para usar Ollama Cloud con dardcor:
...
3. Copie la clave API para usarla en dardcor.
...
6. **Importante**: Antes de usar modelos en la nube en dardcor, debe obtener la información del modelo localmente:
...
opencode Zen es una lista de modelos probados y verificados proporcionada por el equipo dardcor. [Más información](/docs/zen).
...
3. Ingrese su clave dardcor API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración dardcor.
...
5. También puedes personalizarlos a través de tu configuración dardcor. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con dardcor:
...
También puede personalizar modelos a través de su configuración dardcor. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con dardcor. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `dardcor.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a dardcor comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `dardcor auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración dardcor y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración dardcor.
