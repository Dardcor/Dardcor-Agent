...
description: Cree herramientas que LLM pueda llamar en dardcor.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de dardcor como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.dardcor/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/dardcor/tools/`.
...
```ts title=".dardcor/tools/database.ts" {1}
...
```ts title=".dardcor/tools/math.ts"
import { tool } from "@dardcor-ai/plugin"
...
```ts title=".dardcor/tools/bash.ts"
import { tool } from "@dardcor-ai/plugin"
...
```ts title=".dardcor/tools/project.ts" {8}
import { tool } from "@dardcor-ai/plugin"
...
```python title=".dardcor/tools/add.py"
import sys
...
```ts title=".dardcor/tools/python-add.ts" {10}
import { tool } from "@dardcor-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".dardcor/tools/add.py")
