---
title: TUI
description: Usa l'interfaccia utente da terminale (TUI) di dardcor.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

dardcor fornisce un'interfaccia interattiva da terminale (TUI) per lavorare sui tuoi progetti con un LLM.

Eseguire dardcor avvia la TUI per la directory corrente.
...
Quando usi la TUI di dardcor, puoi digitare `/` seguito dal nome di un comando per eseguire rapidamente azioni. Per esempio:
...
Aggiunge un provider a dardcor. Ti permette di selezionare tra i provider disponibili e aggiungere le relative chiavi API.
...
Esce da dardcor. _Alias_: `/quit`, `/q`
...
Questo è separato da `dardcor.json`, che configura il comportamento di server/runtime.
...
- `leader_timeout` - Controlla per quanto tempo dardcor attende dopo il leader key. Default: `2000`.
...
- `sound_pack` - ID del sound pack da usare. Default: `dardcor.default`.
