...
description: Usa la configurazione JSON di dardcor.
---

Puoi configurare dardcor usando un file di configurazione JSON.

---

## Formato

dardcor supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
1. **Config remota** (da `.well-known/dardcor`) - default dell'organizzazione
2. **Config globale** (`~/.config/dardcor/dardcor.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `dardcor_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`dardcor.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.dardcor`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `dardcor_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.dardcor` e `~/.config/dardcor` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/dardcor`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di dardcor in `~/.config/dardcor/dardcor.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.

Per le impostazioni specifiche della TUI, usa `~/.config/dardcor/tui.json`.
...
Aggiungi `dardcor.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando dardcor si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `dardcor_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `dardcor_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.dardcor` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.dardcor`, quindi **puo sovrascrivere** le loro impostazioni.

---

## Schema

Il file di configurazione ha uno schema definito in [**`dardcor.ai/config.json`**](https://dardcor-code.vercel.app//config.json).

La configurazione TUI usa [**`dardcor.ai/tui.json`**](https://dardcor-code.vercel.app//tui.json).
...
Usa `dardcor_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `dardcor.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `dardcor serve` e `dardcor web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server dardcor.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `dardcor.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in dardcor tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, dardcor prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/dardcor/agents/` o `.dardcor/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, dardcor fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`dardcor run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/dardcor/commands/` o `.dardcor/commands/`. [Scopri di piu](/docs/commands).
...
dardcor scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, dardcor **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono dardcor con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.dardcor/plugins/` o `~/.config/dardcor/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare dardcor a usare solo alcuni provider invece di disabilitarli uno per uno.
