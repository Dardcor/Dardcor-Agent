...
Le skill degli agenti permettono a dardcor di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
dardcor cerca in queste posizioni:

- Config di progetto: `.dardcor/skills/<name>/SKILL.md`
- Config globale: `~/.config/dardcor/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, dardcor risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.dardcor/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/dardcor/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.dardcor/skills/git-release/SKILL.md` cosi':
...
dardcor elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `dardcor.json`:
...
**Per agenti integrati** (in `dardcor.json`):
