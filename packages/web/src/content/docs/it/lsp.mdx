---
title: Server LSP
description: dardcor si integra con i tuoi server LSP.
---

dardcor può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

dardcor include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `dardcor_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e dardcor apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di dardcor.
