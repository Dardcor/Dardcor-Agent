...
Puoi aggiungere strumenti esterni a dardcor usando il _Model Context Protocol_, o MCP. dardcor supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [dardcor Config](https://dardcor-code.vercel.app//docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/dardcor`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
dardcor gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, dardcor:
...
Se il server richiede autenticazione, dardcor ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `dardcor mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, dardcor memorizzera' i token in modo sicuro in `~/.local/share/dardcor/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in dardcor insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione dardcor come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare dardcor al tuo account Sentry.
