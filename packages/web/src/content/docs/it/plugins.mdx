---
title: Plugin
description: Scrivi plugin per estendere dardcor.
---

I plugin ti permettono di estendere dardcor agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di dardcor.
...
- `.dardcor/plugins/` - plugin a livello progetto
- `~/.config/dardcor/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/dardcor/node_modules/`.
...
1. Config globale (`~/.config/dardcor/dardcor.json`)
2. Config di progetto (`dardcor.json`)
3. Directory plugin globale (`~/.config/dardcor/plugins/`)
4. Directory plugin di progetto (`.dardcor/plugins/`)
...
dardcor esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di dardcor per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere dardcor.
...
:::note
Se usi l'app desktop di dardcor, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a dardcor di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a dardcor:
...
L'helper `tool` crea uno strumento personalizzato che dardcor può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in dardcor insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://dardcor-code.vercel.app//docs/sdk) per i dettagli.
