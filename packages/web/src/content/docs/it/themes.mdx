...
Con dardcor puoi scegliere tra diversi temi integrati, usare un tema che si adatta al tema del tuo terminale oppure definire un tema personalizzato.

Per impostazione predefinita, dardcor usa il tema `dardcor`.
...
dardcor include diversi temi integrati.
...
- Vuole che dardcor corrisponda all'aspetto del terminale
...
dardcor supporta un sistema di temi flessibile basato su JSON che permette di creare e personalizzare temi facilmente.
...
2. **Directory di configurazione utente** - in `~/.config/dardcor/themes/*.json` o `$XDG_CONFIG_HOME/dardcor/themes/*.json`
3. **Directory root del progetto** - in `<project-root>/.dardcor/themes/*.json`
4. **Directory di lavoro corrente** - in `./.dardcor/themes/*.json`
