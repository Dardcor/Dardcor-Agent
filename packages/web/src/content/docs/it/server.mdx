---
title: Server
description: Interagisci con il server dardcor via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `dardcor serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client dardcor.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `dardcor.local` |
...
Imposta `dardcor_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `dardcor`, oppure imposta `dardcor_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `dardcor serve` sia per `dardcor web`.
...
Quando esegui `dardcor` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server dardcor per interagire con dardcor in modo programmatico.
:::

Questa architettura permette a dardcor di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `dardcor serve` per avviare un server standalone. Se la TUI di dardcor e gia in esecuzione, `dardcor serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di dardcor.
...
Il server dardcor espone le seguenti API.
