...
Agent Skills erlauben dardcor, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
dardcor sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft dardcor vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.dardcor/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/dardcor/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.dardcor/skills/git-release/SKILL.md` zum Beispiel so:
...
dardcor listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `dardcor.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `dardcor.json`):
