...
Mit dardcor kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet dardcor das `dardcor`-Theme.
...
dardcor liefert mehrere eingebaute Themes mit.
...
- dardcor optisch ans Terminal anpassen wollen
...
dardcor unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/dardcor/themes/*.json` oder `$XDG_CONFIG_HOME/dardcor/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.dardcor/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.dardcor/themes/*.json`
