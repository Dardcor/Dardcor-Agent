...
Sie können externe Tools zu dardcor hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. dardcor unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [dardcor Config](https://dardcor-code.vercel.app//docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/dardcor`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
dardcor übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird dardcor Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von dardcor beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `dardcor mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert dardcor die Token sicher in `~/.local/share/dardcor/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in dardcor verfügbar. Sie können sie also wie jedes andere Tool über die dardcor-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und dardcor mit Ihrem Sentry-Konto zu verbinden.
