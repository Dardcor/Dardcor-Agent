---
title: Server
description: Nutze den dardcor-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `dardcor serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein dardcor-Client nutzen kann.
...
Setze `dardcor_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `dardcor`, kann aber mit `dardcor_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `dardcor serve` und `dardcor web`.
...
Wenn du `dardcor` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den dardcor-Server, um dardcor programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `dardcor serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `dardcor serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die dardcor-[IDE](/docs/ide)-Plugins.
...
Der dardcor-Server stellt folgende APIs bereit.
