...
Wenn dardcor Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\dardcor\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `dardcor --log-level DEBUG`.

---

## Speicher

dardcor speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/dardcor/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\dardcor`
...
dardcor Desktop startet im Hintergrund einen lokalen dardcor-Server (`dardcor-cli`-Sidecar).
...
- Nur macOS: `dardcor`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/dardcor/dardcor.jsonc` (oder `~/.config/dardcor/dardcor.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/dardcor/dardcor.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\dardcor\dardcor.jsonc`
...
dardcor kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/dardcor/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\dardcor\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit dardcor ihn neu aufbauen kann.

1. Beende dardcor Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/dardcor`
- **Linux**: loesche `~/.cache/dardcor` (oder fuehre aus: `rm -rf ~/.cache/dardcor`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\dardcor`

3. Starte dardcor Desktop neu.

---

### Server-Verbindungsprobleme beheben

dardcor Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `dardcor.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `dardcor_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `dardcor_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt dardcor Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit dardcor-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

dardcor Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer dardcor in den OS-Einstellungen aktiviert sind, und
...
1. Beende dardcor Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von dardcor Desktop):

- `dardcor.settings.dat` (desktop default server URL)
- `dardcor.global.dat` und `dardcor.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit dardcor hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `dardcor models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\dardcor`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. dardcor installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\dardcor`

2. Starte dardcor neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt dardcor, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
dardcor erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
