---
title: Regeln
description: Lege eigene Anweisungen fuer dardcor fest.
---

Du kannst dardcor ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in dardcor den Befehl `/init` aus.
...
So kann dardcor sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht dardcor sie zu erweitern.
...
dardcor kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/dardcor/AGENTS.md` speichern.
Diese gelten in allen dardcor-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt dardcor dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/dardcor/AGENTS.md` existiert)
...
Beim Start sucht dardcor in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/dardcor/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `dardcor.json` oder in `~/.config/dardcor/dardcor.json` angeben.
...
dardcor parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit dardcor.json

Empfohlen ist das `instructions`-Feld in der `dardcor.json`:
...
Du kannst dardcor explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `dardcor.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
