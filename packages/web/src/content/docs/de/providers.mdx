---
title: Anbieter
description: Verwendung eines beliebigen LLM-Anbieters in dardcor.
---

import config from "../../../../config.mjs"
export const console = config.console

dardcor verwendet [AI SDK](https://ai-sdk.dev/) und [Models.dev](https://models.dev) zur Unterstützung von **75+ LLM-Anbietern** und unterstützt die Ausführung lokaler Modelle.

Um einen Anbieter hinzuzufügen, müssen Sie:

1. Fügen Sie die API-Schlüssel für den Anbieter mit dem Befehl `/connect` hinzu.
2. Konfigurieren Sie den Anbieter in Ihrer dardcor-Konfiguration.
...
Sie können die Anbieter über den Abschnitt `provider` in Ihrem dardcor anpassen
config.
...
opencode Zen ist eine Liste von Modellen, die vom dardcor-Team bereitgestellt wurden
Getestet und verifiziert, dass es gut mit dardcor funktioniert. [Learn more](/docs/zen).

:::tip
Wenn Sie neu sind, empfehlen wir Ihnen, mit opencode Zen zu beginnen.
:::

1. Führen Sie den Befehl `/connect` in TUI aus, wählen Sie dardcor aus und gehen Sie zu [dardcor.ai/auth](https://dardcor-code.vercel.app//auth).
...
Es funktioniert wie jeder andere Anbieter in dardcor und ist völlig optional.

---

## opencode Go

opencode Go ist ein kostenguenstiges Abonnement, das zuverlaessigen Zugriff auf beliebte Open-Coding-Modelle bietet, die vom dardcor-Team getestet und verifiziert wurden, dass sie gut mit dardcor funktionieren.

1. Führen Sie den Befehl `/connect` in der TUI aus, waehlen Sie `opencode Go` und gehen Sie zu [dardcor.ai/auth](https://opencode.ai).
...
So verwenden Sie Amazon Bedrock mit dardcor:
...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Legen Sie eine dieser Umgebungsvariablen fest, während Sie dardcor ausführen:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `dardcor.json`:
...
:::info
Die Verwendung Ihres Claude Pro/Max-Abonnements in dardcor wird von [Anthropic](https://anthropic.com) nicht offiziell unterstützt.
:::
...
Sie können dardcor so konfigurieren, dass es lokale Modelle über [Atomic Chat](https://atomic.chat) verwendet — eine Desktop-Anwendung, die lokale LLMs hinter einem OpenAI-kompatiblen API-Server bereitstellt (Standard-Endpunkt `http://127.0.0.1:1337/v1`).
...
2. Gehen Sie zu [Azure AI Foundry](https://ai.azure.com/) und stellen Sie ein Modell bereit.
   ...
   :::note
   Der Bereitstellungsname muss mit dem Modellnamen übereinstimmen, damit dardcor ordnungsgemäß funktioniert.
   :::
...
5. Führen Sie den Befehl `/models` aus, um ein Modell auszuwählen.
   ...
   Sie können Modelle auch über Ihre dardcor-Konfiguration hinzufügen.
...
um das Token im dardcor-Authentifizierungsspeicher zu speichern.
:::

##### Selbstgehostetes GitLab

:::note[Compliance-Hinweis]
dardcor verwendet ein kleines Modell für einige AI-Aufgaben wie das Generieren des Sitzungstitels.
Es ist standardmäßig für die Verwendung von gpt-5-nano konfiguriert und wird von Zen gehostet. Zum Sperren von dardcor
Um nur Ihre eigene von GitLab gehostete Instanz zu verwenden, fügen Sie Folgendes zu Ihrer hinzu
`dardcor.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
Weitere Dokumentation auf der [dardcor-gitlab-auth](https://www.npmjs.com/package/dardcor-gitlab-auth)-Homepage.

##### Konfiguration

Anpassen über `dardcor.json`:
...
So verwenden Sie Ihr GitHub Copilot-Abonnement mit dardcor:
...
So verwenden Sie Google Vertex AI mit dardcor:
...
2. Legen Sie die erforderlichen Umgebungsvariablen fest:
   ...
   Legen Sie sie fest, während Sie dardcor ausführen.
...
Falls Sie eine Funktion oder ein Modell von Helicone sehen, das nicht automatisch über dardcor konfiguriert wird, können Sie es jederzeit selbst konfigurieren.
...
Mit der [Sessions](https://docs.helicone.ai/features/sessions)-Funktion von Helicone können Sie zusammengehörige LLM-Anfragen gruppieren. Verwenden Sie das [dardcor-helicone-session](https://github.com/H2Shami/dardcor-helicone-session)-Plugin, um jede dardcor-Konversation automatisch als Sitzung in Helicone zu protokollieren.
...
Das Plugin fügt `Helicone-Session-Id`- und `Helicone-Session-Name`-Header in Ihre Anfragen ein. Auf der Seite „Sitzungen“ von Helicone wird jede dardcor-Konversation als separate Sitzung aufgeführt.
...
Sie können dardcor für die Verwendung lokaler Modelle über das Dienstprogramm [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server konfigurieren
...
Sie können dardcor über LM Studio für die Verwendung lokaler Modelle konfigurieren.
...
Sie können dardcor über Ollama für die Verwendung lokaler Modelle konfigurieren.

:::tip
Ollama kann sich automatisch für dardcor konfigurieren. Weitere Informationen finden Sie unter [Ollama integration docs](https://docs.ollama.com/integrations/dardcor).
:::
...
So verwenden Sie Ollama Cloud mit dardcor:
...
3. Kopieren Sie den API-Schlüssel zur Verwendung in dardcor.
...
6. **Wichtig**: Bevor Sie Cloud-Modelle in dardcor verwenden, müssen Sie die Modellinformationen lokal abrufen:
...
opencode Zen ist eine Liste getesteter und verifizierter Modelle, die vom dardcor-Team bereitgestellt werden. [Learn more](/docs/zen).
...
3. Geben Sie Ihren dardcor API-Schlüssel ein.
...
4. Viele OpenRouter-Modelle sind standardmäßig vorinstalliert. Führen Sie den Befehl `/models` aus, um das gewünschte Modell auszuwählen.
   ...
   Sie können auch zusätzliche Modelle über Ihre dardcor-Konfiguration hinzufügen.
...
5. Sie können sie auch über Ihre dardcor-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe eines Anbieters
...
So verwenden Sie [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) mit dardcor:
...
Sie können Modelle auch über Ihre dardcor-Konfiguration anpassen. Hier ist ein Beispiel für die Angabe der Provider-Routing-Reihenfolge.
...
:::tip
Sie können jeden OpenAI-kompatiblen Anbieter mit dardcor verwenden. Die meisten modernen AI-Anbieter bieten OpenAI-kompatible APIs an.
:::
...
4. Erstellen oder aktualisieren Sie Ihre `dardcor.json`-Datei in Ihrem Projektverzeichnis:
...
Mithilfe der `limit`-Felder kann dardcor erkennen, wie viel Kontext Sie noch haben. Standardanbieter beziehen diese automatisch von models.dev.
...
1. **Überprüfen Sie die Authentifizierungseinrichtung**: Führen Sie `dardcor auth list` aus, um zu sehen, ob die Anmeldeinformationen vorhanden sind
...
2. Überprüfen Sie bei benutzerdefinierten Anbietern die dardcor-Konfiguration und:
   - Stellen Sie sicher, dass der im Befehl `/connect` verwendete Anbieter ID mit dem ID in Ihrer dardcor-Konfiguration übereinstimmt.
