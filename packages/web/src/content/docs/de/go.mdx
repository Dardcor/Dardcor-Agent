...
Go funktioniert wie jeder andere Provider in dardcor. Du abonnierst opencode Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
dardcor zu verwenden.
...
opencode Go funktioniert wie jeder andere Provider in dardcor.
...
Die [Modell-ID](/docs/config/#models) in deiner dardcor Config verwendet das Format `dardcor-go/<model-id>`. Für Kimi K2.7 Code würdest du beispielsweise `dardcor-go/kimi-k2.7-code` in deiner Config verwenden.
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit dardcor zu nutzen.
