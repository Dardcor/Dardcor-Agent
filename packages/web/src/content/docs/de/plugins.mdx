---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um dardcor zu erweitern.
---

Mit Plugins können Sie dardcor erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von dardcor zu ändern.
...
- `.dardcor/plugins/` – Plugins auf Projektebene
- `~/.config/dardcor/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/dardcor/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/dardcor/dardcor.json`)
2. Projektkonfiguration (`dardcor.json`)
3. Globales Plugin-Verzeichnis (`~/.config/dardcor/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.dardcor/plugins/`)
...
dardcor führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein dardcor-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie dardcor erweitern können.
...
:::note
Wenn Sie die dardcor-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass dardcor `.env`-Dateien liest:
...
Plugins können dardcor auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das dardcor aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für dardcor zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://dardcor-code.vercel.app//docs/sdk).
