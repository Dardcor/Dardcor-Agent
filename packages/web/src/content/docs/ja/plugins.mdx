---
title: プラグイン
description: dardcor を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで dardcor を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、dardcor のデフォルトの動作を変更したりできます。
...
- `.dardcor/plugins/` - プロジェクトレベルのプラグイン
- `~/.config/dardcor/plugins/` - グローバルプラグイン
...
**npm プラグイン** は、起動時に Bun を使用して自動的にインストールされます。パッケージとその依存関係は `~/.cache/dardcor/node_modules/` にキャッシュされます。
...
1. グローバル設定 (`~/.config/dardcor/dardcor.json`)
2. プロジェクト設定 (`dardcor.json`)
3. グローバルプラグインディレクトリ (`~/.config/dardcor/plugins/`)
4. プロジェクトプラグインディレクトリ (`.dardcor/plugins/`)
...
dardcor は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための dardcor SDK クライアント。
...
ここでは、dardcor を拡張するために使用できるプラグインの例をいくつか示します。
...
レベル: `debug`、`info`、`warn`、`error`。詳細については、[SDK ドキュメント](https://dardcor-code.vercel.app//docs/sdk) を参照してください。
...
dardcor が `.env` ファイルを読み取らないようにします。
...
プラグインは dardcor にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、dardcor が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して dardcor で使用できます。
