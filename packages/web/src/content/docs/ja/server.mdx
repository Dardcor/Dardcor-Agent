---
title: サーバー
description: HTTP 経由で dardcor サーバーと通信します。
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`dardcor serve` コマンドは、dardcor クライアントが使用できる OpenAPI エンドポイントを公開するヘッドレス HTTP サーバーを実行します。
...
| `--mdns-domain` | mDNS サービスのカスタムドメイン名 | `dardcor.local` |
...
HTTP 基本認証でサーバーを保護するには、`dardcor_SERVER_PASSWORD` を設定します。ユーザー名はデフォルトで `dardcor` になるか、`dardcor_SERVER_USERNAME` を設定してオーバーライドします。これは、`dardcor serve` と `dardcor web` の両方に当てはまります。
...
`dardcor` を実行すると、TUI とサーバーが起動します。 TUI はサーバーと通信するクライアントです。サーバーは OpenAPI 3.1 仕様のエンドポイントを公開します。このエンドポイントは、[SDK](/docs/sdk) を使用してアクセスできます。

:::tip
dardcor サーバーを使用して、プログラムで dardcor と対話します。
:::

`dardcor serve` を実行してスタンドアロンサーバーを起動できます。すでに dardcor TUI を実行している場合でも、`dardcor serve` は新しいサーバーを起動します。
...
[`/tui`](#tui) エンドポイントは、サーバー経由で TUI を駆動するために使用できます。たとえば、プロンプトを事前入力したり、実行したりできます。この設定は、dardcor [IDE](/docs/ide) プラグインによって使用されます。
