---
title: Server
description: Komunicirajte s dardcor serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `dardcor serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji dardcor klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `dardcor.local` |
...
Postavite `dardcor_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `dardcor`, ili postavite `dardcor_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `dardcor serve` i za `dardcor web`.
...
Kada pokrenete `dardcor`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite dardcor server za programsku interakciju sa dardcor-om.
:::

Ova arhitektura omogucava dardcor podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `dardcor serve` da startate standalone server. Ako je dardcor TUI vec pokrenut, `dardcor serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste dardcor [IDE](/docs/ide) pluginovi.
...
dardcor server izlaže sljedece API-je.
