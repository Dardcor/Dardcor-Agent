---
title: Dodaci
description: Napišite vlastite dodatke za proširenje dardcor.
---

Dodaci vam omogućavaju da proširite dardcor spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja dardcor.
...
- `.dardcor/plugins/` - Dodaci na nivou projekta
- `~/.config/dardcor/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/dardcor/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/dardcor/dardcor.json`)
2. Konfiguracija projekta (`dardcor.json`)
3. Globalni direktorij dodataka (`~/.config/dardcor/plugins/`)
4. Direktorij dodataka projekta (`.dardcor/plugins/`)
...
dardcor pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: dardcor SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje dardcor.
...
Spriječite dardcor da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u dardcor:
...
Pomoćnik `tool` kreira prilagođeni alat koji dardcor može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za dardcor zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://dardcor-code.vercel.app//docs/sdk) za detalje.
