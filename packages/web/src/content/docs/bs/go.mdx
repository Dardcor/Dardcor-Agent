...
Go radi kao bilo koji drugi provajder u dardcor-u. Pretplatite se na opencode Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili dardcor.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa dardcor-om.
:::
...
opencode Go radi kao bilo koji drugi provajder u dardcor-u.
...
[Model id](/docs/config/#models) u vašoj dardcor konfiguraciji
koristi format `dardcor-go/<model-id>`. Na primjer, za Kimi K2.7 Code, koristili biste
`dardcor-go/kimi-k2.7-code` u svojoj konfiguraciji.
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz dardcor koristite i bilo kojeg drugog provajdera.
