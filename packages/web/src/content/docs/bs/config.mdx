---
title: Konfiguracija
description: Korištenje dardcor JSON konfiguracije.
---

Možete konfigurirati dardcor koristeći JSON konfiguracijski fajl.

---

## Format

dardcor podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/dardcor`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/dardcor/dardcor.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`dardcor_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`dardcor.json` u projektu) - postavke specifične za projekat
5. **`.dardcor` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`dardcor_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.dardcor` i `~/.config/dardcor` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/dardcor` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu dardcor konfiguraciju u `~/.config/dardcor/dardcor.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/dardcor/tui.json`.
...
Dodajte `dardcor.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se dardcor pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `dardcor_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `dardcor_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.dardcor` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.dardcor`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`dardcor.ai/config.json`**](https://dardcor-code.vercel.app//config.json).

TUI konfiguracija koristi [**`dardcor.ai/tui.json`**](https://dardcor-code.vercel.app//tui.json).
...
Koristite `dardcor_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `dardcor.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `dardcor serve` i `dardcor web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš dardcor server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `dardcor.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj dardcor konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, dardcor pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/dardcor/agents/` ili `.dardcor/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, dardcor će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`dardcor run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/dardcor/commands/` ili `.dardcor/commands/`. [Saznajte više ovdje](/docs/commands).
...
dardcor će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, dardcor **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju dardcor sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.dardcor/plugins/` ili `~/.config/dardcor/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite dardcor da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
