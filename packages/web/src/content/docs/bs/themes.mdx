...
U dardcor mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, dardcor koristi nasu `dardcor` temu.
...
dardcor dolazi sa vise ugradenih tema.
...
- Zele da dardcor odgovara izgledu njihovog terminala
...
dardcor podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/dardcor/themes/*.json` ili `$XDG_CONFIG_HOME/dardcor/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.dardcor/themes/*.json`
4. **Trenutni radni direktorij** - `./.dardcor/themes/*.json`
