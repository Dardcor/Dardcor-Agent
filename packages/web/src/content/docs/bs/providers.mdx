---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u dardcor.
---

import config from "../../../../config.mjs"
export const console = config.console

dardcor koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj dardcor konfiguraciji.
...
u `~/.local/share/dardcor/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem dardcor
config.
...
opencode Zen je lista modela koje je obezbedio dardcor tim koji su bili
testirano i potvrđeno da dobro radi sa dardcor. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa opencode Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite dardcor i idite na [dardcor.ai/auth](https://dardcor-code.vercel.app//auth).
...
Radi kao i svaki drugi provajder u dardcor i potpuno je opcionalan za korištenje.

---

## opencode Go

opencode Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža dardcor tim i koji su testirani i verificirani da dobro rade s dardcor-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `opencode Go` i idite na [dardcor.ai/auth](https://opencode.ai).
...
Da biste koristili Amazon Bedrock s dardcor:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete dardcor:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `dardcor.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u dardcor.
:::
...
Možete konfigurirati dardcor za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi dardcor ispravno radio.
   :::
...
Također možete dodati modele kroz svoju dardcor konfiguraciju.
...
da pohrani token u dardcor auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
dardcor koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate dardcor
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`dardcor.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [dardcor-gitlab-auth](https://www.npmjs.com/package/dardcor-gitlab-auth).

##### Konfiguracija

Prilagodite putem `dardcor.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s dardcor:
...
Za korištenje Google Vertex AI s dardcor:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće dardcor.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem dardcora, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [dardcor-helicone-session](https://github.com/H2Shami/dardcor-helicone-session) da automatski prijavite svaki dardcor razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki dardcor razgovor naveden kao zasebna sesija.
...
Možete konfigurirati dardcor za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati dardcor za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati dardcor za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za dardcor. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/dardcor) za detalje.
:::
...
Da biste koristili Ollama Cloud s dardcor:
...
3. Kopirajte API ključ za korištenje u dardcor.
...
6. **Važno**: Prije upotrebe modela oblaka u dardcor, morate lokalno povući informacije o modelu:
...
opencode Zen je lista testiranih i verifikovanih modela koju je obezbedio dardcor tim. [Saznajte više](/docs/zen).
...
3. Unesite svoj dardcor API ključ.
...
Također možete dodati dodatne modele putem vaše dardcor konfiguracije.
...
5. Također ih možete prilagoditi putem vaše dardcor konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa dardcorom:
...
Također možete prilagoditi modele kroz svoju dardcor konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s dardcor-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `dardcor.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju dardcor da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `dardcor auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite dardcor konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj dardcor konfiguraciji.
