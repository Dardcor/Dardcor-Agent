...
Možete dodati vanjske alate u dardcor koristeći _Model Context Protocol_, ili MCP. dardcor podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [dardcor Config](https://dardcor-code.vercel.app//docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/dardcor`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
dardcor automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, dardcor će:
...
Ako server zahtijeva autentifikaciju, dardcor će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `dardcor mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, dardcor sigurno čuva tokene u `~/.local/share/dardcor/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u dardcor, zajedno s ugrađenim alatima. Možete njima upravljati kroz dardcor konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete dardcor sa vašim Sentry nalogom.
