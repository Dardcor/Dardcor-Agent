...
Da biste otklonili probleme s dardcor, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\dardcor\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `dardcor --log-level DEBUG`.

---

## Pohrana

dardcor pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/dardcor/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\dardcor`
...
dardcor Desktop pokreće lokalni dardcor server (`dardcor-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `dardcor` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/dardcor/dardcor.jsonc` (ili `~/.config/dardcor/dardcor.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/dardcor/dardcor.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\dardcor\dardcor.jsonc`
...
dardcor također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/dardcor/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\dardcor\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga dardcor mogao ponovo izgraditi.

1. Potpuno zatvorite dardcor Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/dardcor`
- **Linux**: obrišite `~/.cache/dardcor` (ili pokrenite `rm -rf ~/.cache/dardcor`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\dardcor`

3. Ponovo pokrenite dardcor Desktop.

---

### Rješavanje problema sa vezom na serveru

dardcor Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `dardcor.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `dardcor_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `dardcor_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, dardcor Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa dardcor karakteristikama.

---

### Obavještenja se ne prikazuju

dardcor Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za dardcor u postavkama vašeg OS-a, i
...
1. Zatvorite dardcor Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka dardcor Desktop aplikacije):

- `dardcor.settings.dat` (URL zadanog servera za desktop)
- `dardcor.global.dat` i `dardcor.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `dardcor models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\dardcor`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. dardcor dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\dardcor`

2. Ponovo pokrenite dardcor da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti dardcor da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
dardcor će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
