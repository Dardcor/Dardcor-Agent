...
Agent skills omogucavaju dardcor da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
dardcor pretrazuje ove lokacije:

- Konfiguracija projekta: `.dardcor/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/dardcor/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, dardcor ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.dardcor/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/dardcor/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.dardcor/skills/git-release/SKILL.md` ovako:
...
dardcor navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `dardcor.json`:
...
**Za ugradene agente** (u `dardcor.json`):
