...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. dardcor dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u dardcor.
...
:::note
Ovaj alat je dostupan samo kada je `dardcor_EXPERIMENTAL_LSP_TOOL=true` (ili `dardcor_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz dardcor provajdera ili kada je varijabla `dardcor_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju dardcor:
