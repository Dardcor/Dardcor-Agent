...
Med dardcor kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger dardcor vårt eget `dardcor`-tema.
...
dardcor kommer med flere innebygde temaer.
...
- Vil at dardcor skal matche terminalens utseende
...
dardcor støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/dardcor/themes/*.json` eller `$XDG_CONFIG_HOME/dardcor/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.dardcor/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.dardcor/themes/*.json`
