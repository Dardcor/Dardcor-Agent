---
title: Plugins
description: Skriv dine egne plugins for at udvide dardcor.
---

Plugins giver dig mulighed for at udvide dardcor ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre dardcors standardadfærd.
...
- `.dardcor/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/dardcor/node_modules/`.
...
1. Global konfiguration (`~/.config/dardcor/dardcor.json`)
2. Projektkonfiguration (`dardcor.json`)
3. Global plugin-mappe (`~/.config/dardcor/plugins/`)
4. Projekt plugin bibliotek (`.dardcor/plugins/`)
...
dardcor kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En dardcor SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide dardcor.
...
:::note
Hvis du bruger dardcor desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå dardcor i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til dardcor:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som dardcor kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for dardcor sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://dardcor-code.vercel.app//docs/sdk) for detaljer.
