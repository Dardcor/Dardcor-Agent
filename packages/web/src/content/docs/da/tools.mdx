...
Her er alle de innebygde verktøyene som er tilgængelige i dardcor.
...
:::note
Dette verktøyet er bare tilgængelig når `dardcor_EXPERIMENTAL_LSP_TOOL=true` (eller `dardcor_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger dardcor-leverandøren eller når miljøvariabelen `dardcor_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
