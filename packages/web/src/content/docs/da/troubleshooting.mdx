...
For at fejlfinde problemer med dardcor, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\dardcor\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `dardcor --log-level DEBUG`.

---

## Lagring

dardcor gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/dardcor/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\dardcor`
...
dardcor Desktop kører en lokal dardcor-server (`dardcor-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `dardcor`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/dardcor/dardcor.jsonc` (eller `~/.config/dardcor/dardcor.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/dardcor/dardcor.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\dardcor\dardcor.jsonc`
...
dardcor kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/dardcor/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\dardcor\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så dardcor kan genopbygge den.

1. Afslut dardcor Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/dardcor`
- **Linux**: slet `~/.cache/dardcor` (eller kør `rm -rf ~/.cache/dardcor`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\dardcor`

3. Start dardcor Desktop på ny.

---

### Løs problemer med serverforbindelse

dardcor Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `dardcor.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `dardcor_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `dardcor_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver dardcor Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i dardcor.

---

### Meddelelser vises ikke

dardcor Desktop viser kun systemvarsler når:

- varsler er aktiveret for dardcor i dine OS-indstillinger, og
...
1. Afslut dardcor Desktop.
2. Find og slet disse filer (de findes i dardcor Desktop-appens datamappe):

- `dardcor.settings.dat` (skrivebordsstandardserver URL)
- `dardcor.global.dat` og `dardcor.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med dardcor:
...
For at finde ud af hvilke modeller du har adgang til, kør `dardcor models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\dardcor`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. dardcor installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\dardcor`

2. Start dardcor på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge dardcor til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
dardcor vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
