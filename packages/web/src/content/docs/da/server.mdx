---
title: Server
description: Interager med dardcor-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `dardcor serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en dardcor-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `dardcor.local` |
...
Indstil `dardcor_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `dardcor`, eller indstil `dardcor_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `dardcor serve` og `dardcor web`.
...
Når du kører `dardcor` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug dardcor-serveren til at interagere med dardcor programmatisk.
:::

Denne arkitektur lader dardcor understøtte flere klienter og lader dig interagere med dardcor programmatisk.

Du kan køre `dardcor serve` for at starte en selvstændig server. Hvis du har
dardcor TUI kørende, vil `dardcor serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af dardcor [IDE](/docs/ide) plugins.
...
dardcor-serveren viser følgende API'er.
