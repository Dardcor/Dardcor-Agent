---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i dardcor.
---

import config from "../../../../config.mjs"
export const console = config.console

dardcor bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i dardcor-konfigurationen.

---

### Legitimationsoplysninger

Når du tilføjer en udbyders API-nøgler med `/connect`-kommandoen, gemmes de i `~/.local/share/dardcor/auth.json`.

---

### Konfiguration

Du kan tilpasse udbyderne gennem `provider`-delen i dardcor-konfigurationen.
...
opencode Zen er en liste over modeller leveret af dardcor-teamet, som er blevet testet og verificeret til at fungere godt med dardcor. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med opencode Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg dardcor og gå til [dardcor.ai/auth](https://dardcor-code.vercel.app//auth).
...
Det fungerer som alle andre udbydere i dardcor og er helt valgfrit at bruge.

---

## opencode Go

opencode Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af dardcor-teamet, som er testet og verificeret til at fungere godt med dardcor.

1. Kør kommandoen `/connect` i TUI, vælg `opencode Go`, og gå til [dardcor.ai/auth](https://opencode.ai).
...
Sådan bruger du Amazon Bedrock med dardcor:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører dardcor:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `dardcor.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i dardcor understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere dardcor til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at dardcor skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem dardcor-konfigurationen.
...
gemme tokenet i dardcor auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
dardcor bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse dardcor
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`dardcor.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [dardcor-gitlab-auth](https://www.npmjs.com/package/dardcor-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `dardcor.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med dardcor:
...
Sådan bruger du Google Vertex AI med dardcor:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem dardcor, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [dardcor-helicone-session](https://github.com/H2Shami/dardcor-helicone-session) plugin for automatisk at logge hver dardcor-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver dardcor-samtale opført som en separat session.
...
Du kan konfigurere dardcor til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere dardcor til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere dardcor til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for dardcor. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/dardcor) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med dardcor:
...
3. Kopier API-nøglen til brug i dardcor.
...
6. **Vigtigt**: Før du bruger skymodeller i dardcor, skal du hente modelinformationen lokalt:
...
opencode Zen er en liste over testede og verificerede modeller leveret af dardcor-teamet. [Læs mere](/docs/zen).
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem dardcor-konfigurationen.
...
5. Du kan også tilpasse dem gennem dardcor-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med dardcor:
...
Du kan også tilpasse modeller gennem dardcor-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med dardcor. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `dardcor.json`-filen i projektkataloget:
...
`limit`-felterne lader dardcor forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `dardcor auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek dardcor-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i dardcor-konfigurationen.
