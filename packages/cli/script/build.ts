...
for (const item of targets) {
  ...
  const result = await Bun.build({
    ...
    define: {
      dardcor_VERSION: `'${Script.version}'`,
      dardcor_CLI_NAME: `'${binary}'`,
      dardcor_MODELS_DEV: modelsData,
      dardcor_CHANNEL: `'${Script.channel}'`,
      dardcor_LIBC: item.os === "linux" ? `'${item.abi ?? "glibc"}'` : "undefined",
  ...
  await Bun.write(
    ...
    JSON.stringify(
      {
        name: `@dardcor-ai/${name}`,
        version: Script.version,
        license: "MIT",
        repository: { type: "git", url: "git+https://github.com/anomalyco/dardcor.git" },
        os: [item.os],
        cpu: [item.arch],
      },
      null,
