...
import { PromptInput } from "@dardcor-ai/schema/prompt-input"
import { Session } from "@dardcor-ai/schema/session"
import { Project } from "@dardcor-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@dardcor-ai/schema/schema"
...
import { SessionEvent } from "@dardcor-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this dardcor process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this dardcor process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
