...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when dardcor needs attention", () => {
    expect(wsldardcorAction(undefined)).toBeUndefined()
    expect(
      wsldardcorAction({
        distro: "Debian",
        resolvedPath: null,
    ...
    ).toBe("Install dardcor")
    expect(
      wsldardcorAction({
        distro: "Debian",
        resolvedPath: "/usr/local/bin/dardcor",
        version: "1.2.2",
        expectedVersion: "1.2.3",
  ...
  test("does not report ready when dardcor is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        dardcorChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.dardcor/bin/dardcor",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.distroStatuses.Debian).toEqual({
      label: { key: "wsl.onboarding.installdardcor" },
      tone: "warning",
    })
    expect(model.primaryButton.action).toBe("install-dardcor")
  })
