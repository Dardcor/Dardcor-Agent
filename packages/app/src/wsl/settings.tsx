...
import { Tag } from "@dardcor-ai/ui/v2/badge-v2"
...
import { wsldardcorAction, wslRuntimeRetryable } from "./settings-model"
    ...
    <Show when={api}>
      <For each={props.servers()}>
        {(item) => {
          ...
          const check = () => wsl.data?.dardcorChecks[item.config.distro]
          const dardcorAction = () => wsldardcorAction(check())
          const busy = () => wsl.data?.job?.kind === "install-dardcor" && wsl.data.job.distro === item.config.distro
          return (
            <div class="settings-v2-servers-row">
              ...
              <div class="settings-v2-servers-actions">
                ...
                <Show when={dardcorAction()}>
                  {(label) => (
                    <ButtonV2
                      size="small"
                      disabled={busy() || request.isPending}
                      onClick={() => api && request.mutate(() => api.installdardcor(item.config.distro))}
                    >
