...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-dardcor" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wsldardcorAction(check?: WsldardcorCheck) {
  if (!check) return
  if (!check.resolvedPath) return "Install dardcor"
  if (check.matchesDesktop === false) return "Update dardcor"
}
...
export function addServerViewModel(input: {
  ...
  const dardcorCheck = selectedDistro ? (state?.dardcorChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.dardcorChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (check.matchesDesktop === false) return { label: { key: "wsl.onboarding.updatedardcor" }, tone: "warning" }
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.dardcorMissing" }, tone: "warning" }
  if (check.error) return { label: { key: "wsl.onboarding.installdardcor" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  state: WslServersState | undefined
  selectedDistro: string | null
  dardcorCheck: WsldardcorCheck | null
  adding: boolean
  probingAddable: boolean
  ...
  const installingdardcor =
    input.state?.job?.kind === "install-dardcor" && input.state.job.distro === input.selectedDistro
  if (!ready || probingdardcor) {
    return {
      variant: "contrast",
  ...
  if (!addServerdardcorReady(input.dardcorCheck)) {
    const update = !!input.dardcorCheck?.resolvedPath && input.dardcorCheck.matchesDesktop === false
    return {
      variant: "neutral",
      label: installingdardcor
        ? { key: "wsl.onboarding.updatingdardcor" }
        : update
          ? { key: "wsl.onboarding.updatedardcor" }
          : { key: "wsl.onboarding.installdardcor" },
      disabled: !!input.state?.job || input.adding,
      action: "install-dardcor",
      loading: installingdardcor,
...
function addServerdardcorReady(check: WsldardcorCheck | null) {
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.dardcorChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.dardcorChecks[item.name]) return [`dardcor:${item.name}`]
    return []
  })
