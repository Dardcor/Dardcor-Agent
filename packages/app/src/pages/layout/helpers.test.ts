...
import { type Session } from "@dardcor-ai/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("dardcor://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("dardcor://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("dardcor://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("dardcor://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("dardcor://open-project")).toBeUndefined()
    expect(parseDeepLink("dardcor://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "dardcor://open-project?directory=/a",
      "dardcor://other?directory=/b",
      "dardcor://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("dardcor://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("dardcor://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("dardcor://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("dardcor://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "dardcor://new-session?directory=/a",
      "dardcor://open-project?directory=/b",
      "dardcor://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __dardcor__: {
        deepLinks: ["dardcor://open-project?directory=/a"],
      },
    } as unknown as Window & { __dardcor__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["dardcor://open-project?directory=/a"])
