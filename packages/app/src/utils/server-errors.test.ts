...
import type { SessionNotFoundError } from "@dardcor-ai/sdk/v2/client"
...
describe("parseReadableConfigInvalidError", () => {
  test("formats issues with file path", () => {
    ...
    expect(result).toBe(
      ["Arquivo de config em dardcor.config.ts invalido: settings.host: Required", "mode: Invalid"].join("\n"),
    )
  })
