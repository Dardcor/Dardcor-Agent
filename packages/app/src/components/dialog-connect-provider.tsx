import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@dardcor-ai/sdk/v2/client"
...
import { List, type ListRef } from "@dardcor-ai/ui/list"
import { ProviderIcon } from "@dardcor-ai/ui/provider-icon"
import { Spinner } from "@dardcor-ai/ui/spinner"
import { Tag } from "@dardcor-ai/ui/tag"
...
function ProviderPicker(props: { directory?: Accessor<string | undefined>; onSelect: (provider: string) => void }) {
  ...
  const note = (id: string) => {
    ...
    if (id === "dardcor-go") return language.t("dialog.provider.dardcorGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "dardcor"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.dardcor.tagline")}</div>
          </Show>
  ...
  function ApiAuthView() {
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "dardcor"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.dardcorZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.dardcorZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.dardcorZen.visit.prefix")}
                <Link href="https://opencode.ai" tabIndex={-1}>
                  {language.t("provider.connect.dardcorZen.visit.link")}
                </Link>
                {language.t("provider.connect.dardcorZen.visit.suffix")}
              </div>
            </div>
          </Match>
          <Match when={true}>
