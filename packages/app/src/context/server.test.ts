  ...
  test("moves an existing canonical web bucket into local scope", () => {
    expect(
      migrateCanonicalLocalServerState(
        {
          list: [],
          projects: { "https://dardcor.example.com": [{ worktree: "/remote", expanded: true }] },
          lastProject: { "https://dardcor.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://dardcor.example.com"),
      ),
    ).toEqual({
      list: [],
      ...
      migrateCanonicalLocalServerState(
        {
          projects: {
            ...
            "https://dardcor.example.com": [
              { worktree: "/local", expanded: true },
              { worktree: "/remote", expanded: true },
            ],
          },
          lastProject: { local: "/local", "https://dardcor.example.com": "/remote" },
        },
        ServerConnection.Key.make("https://dardcor.example.com"),
      ),
    ).toEqual({
