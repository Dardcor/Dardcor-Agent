...
import { SessionStatusEvent } from "@dardcor-ai/schema/session-status-event"
import { SessionV1 } from "@dardcor-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
import { mockdardcorServer } from "../../utils/mock-server"
import { installSseTransport } from "../../utils/sse-transport"
import { expectSessionTitle } from "../../utils/waits"

export const directory = "C:/dardcor/TimelineStability"
...
export const model = { providerID: "dardcor", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("dardcor.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
...
function provider() {
  return {
    ...
    connected: ["dardcor"],
    default: { providerID: "dardcor", modelID: "claude-opus-4-6" },
  }
}
