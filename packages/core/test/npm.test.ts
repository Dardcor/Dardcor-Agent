...
describe("Npm.sanitize", () => {
  test("keeps normal scoped package specs unchanged", () => {
    expect(Npm.sanitize("@dardcor/acme")).toBe("@dardcor/acme")
    expect(Npm.sanitize("@dardcor/acme@1.0.0")).toBe("@dardcor/acme@1.0.0")
    expect(Npm.sanitize("prettier")).toBe("prettier")
  })

  test("handles git https specs", () => {
    const spec = "acme@git+https://github.com/dardcor/acme.git"
    const expected = win ? "acme@git+https_//github.com/dardcor/acme.git" : spec
