...
import { define } from "@dardcor-ai/plugin/v2/effect"
...
import { LocationServiceMap } from "@dardcor-ai/core/location-services"
      ...
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(blocked.path, "dardcor.json"),
              JSON.stringify({
    ...
    Effect.acquireRelease(
      ...
      Effect.flatMap((dir) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(
              path.join(dir.path, "dardcor.json"),
              JSON.stringify({
                providers: {
                  unavailable: {
                    name: "Unavailable",
