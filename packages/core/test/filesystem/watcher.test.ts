...
import { Watcher } from "@dardcor-ai/core/filesystem/watcher"
...
const flagsLayer = ConfigProvider.layer(
  ConfigProvider.fromUnknown({
    dardcor_EXPERIMENTAL_FILEWATCHER: "true",
    dardcor_EXPERIMENTAL_DISABLE_FILEWATCHER: "false",
  }),
)
...
function withTmp<A, E, R>(
  ...
  return Effect.acquireRelease(
    Effect.promise(async () => {
      ...
      await $`git config user.email test@dardcor.test`.cwd(tmp.path).quiet()
