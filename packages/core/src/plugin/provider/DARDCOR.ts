...
import type { IntegrationOAuthMethodRegistration } from "@dardcor-ai/plugin/v2/effect/integration"
import { define } from "@dardcor-ai/plugin/v2/effect/plugin"
import type { CredentialValue } from "@dardcor-ai/sdk/v2/types"
...
const defaultServer = "https://console.dardcor.ai"
const clientID = "dardcor-cli"
...
export const dardcorPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "dardcor",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("dardcorPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("dardcor")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load dardcor provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("dardcor", (integration) => {
        integration.name = "dardcor"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "dardcor", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("dardcor")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("dardcor")
      ...
      const item = catalog.provider.get(ProviderV2.ID.dardcor)
      if (!item) return
      const hasKey = Boolean(process.env.dardcor_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("dardcor")),
