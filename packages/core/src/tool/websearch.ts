...
import { ToolFailure } from "@dardcor-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@dardcor/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.dardcor_WEBSEARCH_PROVIDER === "exa" || process.env.dardcor_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.dardcor_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("dardcor_EXPERIMENTAL") || truthy("dardcor_ENABLE_EXA") || truthy("dardcor_EXPERIMENTAL_EXA"),
    enableParallel: truthy("dardcor_ENABLE_PARALLEL") || truthy("dardcor_EXPERIMENTAL_PARALLEL"),
              ...
              const text =
                provider === "exa"
                  ...
                  : yield* callMcp(
                      ...
                      {
                        "User-Agent": `dardcor/${InstallationVersion}`,
