...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve dardcor route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "dardcor")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "dardcor")).toBe("openai")
    expect(statProvider("big-pickle", "", "dardcor")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "dardcor"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep dardcor as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "dardcor"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "dardcor"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep dardcor or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "dardcor"), country: "US" })).toMatchObject([
