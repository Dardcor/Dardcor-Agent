...
import { and, Database, eq, isNull, lt, or, sql } from "@dardcor-ai/console-core/drizzle/index.js"
import { KeyTable } from "@dardcor-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@dardcor-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@dardcor-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@dardcor-ai/console-core/util/date.js"
import { Identifier } from "@dardcor-ai/console-core/identifier.js"
import { Billing } from "@dardcor-ai/console-core/billing.js"
import { Actor } from "@dardcor-ai/console-core/actor.js"
import { WorkspaceTable } from "@dardcor-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@dardcor-ai/console-core/model.js"
import { Subscription } from "@dardcor-ai/console-core/subscription.js"
import { BlackData } from "@dardcor-ai/console-core/black.js"
import { UserTable } from "@dardcor-ai/console-core/schema/user.sql.js"
import { ModelTable } from "@dardcor-ai/console-core/schema/model.sql.js"
import { ProviderTable } from "@dardcor-ai/console-core/schema/provider.sql.js"
...
import { LiteData } from "@dardcor-ai/console-core/lite.js"
import { Resource } from "@dardcor-ai/console-resource"
...
import { Workspace } from "@dardcor-ai/console-core/workspace.js"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-dardcor-session") ?? ""
    const requestId = input.request.headers.get("x-dardcor-request") ?? ""
    const ocClient = input.request.headers.get("x-dardcor-client") ?? ""
    const projectId = input.request.headers.get("x-dardcor-project") ?? ""
      ...
      if (!allowedRegions?.includes("unavailable"))
        throw new RegionError(
          t("zen.api.error.regionNotAllowed", {
            consoleGoUrl: `https://dardcor-code.vercel.app//workspace/${authInfo.workspaceID}/go`,
          }),
        )
    }
    */
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWith429Retry(
        reqUrl,
        {
          ...
          headers: (() => {
            ...
            headers.delete("x-dardcor-request")
            headers.delete("x-dardcor-session")
            headers.delete("x-dardcor-project")
            headers.delete("x-dardcor-client")
            return headers
          })(),
          body: reqBody,
      ...
      if (isNewInference) {
        const resEndpointId = res.headers.get("x-dardcor-endpoint-id")
        const resEndpointModelId = res.headers.get("x-dardcor-upstream-model-id")
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://dardcor-code.vercel.app//workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://dardcor-code.vercel.app//workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://dardcor-code.vercel.app//workspace/${authInfo.workspaceID}/members`
