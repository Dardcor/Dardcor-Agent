...
describe("translate app", () => {
  ...
  test("maps product locale codes to their glossaries", () => {
    expect(glossaryFile("fr")).toBe(".dardcor/glossary/fr.md")
    expect(glossaryFile("zh")).toBe(".dardcor/glossary/zh-cn.md")
    expect(glossaryFile("zht")).toBe(".dardcor/glossary/zh-tw.md")
  })
  ...
  test("reads the actual model and variant from the completed session", () => {
    ...
    ).toEqual([{ model: "dardcor/gpt-5.5", variant: "xhigh" }])
    expect(
      textFromEvents(
...
{"variants":{}}
`
    expect(modelVariants(output, "dardcor/gpt-5.5")).toEqual({
  ...
  test("disables side effects and scopes edits for the translation agent", () => {
    const config = translationConfig("translate-app-fr", "dardcor/gpt-5.5", ["packages/app/src/i18n/fr.ts"])
