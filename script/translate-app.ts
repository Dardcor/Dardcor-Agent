...
export function parseTranslationArgs(args: string[]) {
  const parsed = parseArgs({
    ...
    options: {
      ...
      model: { type: "string", default: "dardcor/gpt-5.5" },
...
export function glossaryFile(locale: Locale) {
  if (locale === "zh") return ".dardcor/glossary/zh-cn.md"
  if (locale === "zht") return ".dardcor/glossary/zh-tw.md"
  return `.dardcor/glossary/${locale}.md`
}
...
export function sessionIDFromEvents(output: string) {
  ...
  if (!match?.[1]) throw new Error("dardcor did not report a session ID.")
  return match[1]
}

export function sessionModels(value: unknown) {
  if (!isRecord(value) || !Array.isArray(value.messages))
    throw new Error("dardcor returned an invalid session export.")
  return value.messages.flatMap((message) => {
    ...
    if (typeof message.info.providerID !== "string" || typeof message.info.modelID !== "string") {
      throw new Error("dardcor session export omitted the assistant model.")
    }
    return [
      {
...
Options:
  -c, --concurrency <count>  Maximum parallel dardcor runs for 'all' (default: 4)
      --model <provider/id>  dardcor model (default: dardcor/gpt-5.5)
      --variant <name>       Model variant (default: xhigh)
      --dry-run              Report drift without running dardcor
...
`)
  ...
  if (failed.length) console.error(`\ndardcor failed for: ${failed.map((result) => result.locale).join(", ")}`)
...
async function translate(
  ...
  env.dardcor_DISABLE_PROJECT_CONFIG = "1"
  env.dardcor_CONFIG_CONTENT = JSON.stringify(
    translationConfig(
      agent,
      model,
  ...
  const exported = Bun.spawn(["dardcor", "--pure", "export", sessionID, "--sanitize"], {
    cwd: root,
    env,
    stdout: "pipe",
    stderr: "pipe",
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  const env = isolatedEnvironment()
  env.dardcor_DISABLE_PROJECT_CONFIG = "1"
  const proc = Bun.spawn(["dardcor", "--pure", "models", provider, "--verbose"], {
    cwd: root,
    env,
    stdin: "ignore",
    stdout: "pipe",
...
function isolatedEnvironment() {
  ...
  delete env.dardcor_CONFIG
  delete env.dardcor_CONFIG_DIR
  delete env.dardcor_CONFIG_CONTENT
  delete env.dardcor_PERMISSION
  delete env.dardcor_AUTO_SHARE
  return env
}
