...
import { createdardcor } from "@dardcor-ai/sdk"
...
`)
  ...
  const dardcor = await createdardcor({ port: 0 })
  ...
  try {
    ...
    const session = await dardcor.client.session.create()
    const result = await dardcor.client.session
      .prompt({
