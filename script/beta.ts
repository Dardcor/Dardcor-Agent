...
const model = "dardcor/gpt-5.3-codex"
...
async function build() {
  ...
  try {
    await $`./script/build.ts --single`.cwd("packages/dardcor")
    return true
  } catch (err) {
...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with dardcor...`)
  ...
  try {
    await $`dardcor run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  dardcor failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with dardcor")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with dardcor...")
  ...
  const prompt = [
    ...
    "Run `./script/build.ts --single` in `packages/dardcor`.",
