...
// ─── Model Database ──────────────────────────────────────────────────────────
// tags: free, fast, cheap, local, vision, code, reason, long
const MODELS = {
...
export async function handleModels(opts = {}) {
  ...
  const filter  = opts.filter   || opts.f   || '';   // --filter code
  const search  = opts.search   || opts.s   || '';   // --search gpt
  const provOpt = opts.provider || opts.p   || '';   // --provider openai
  const pricing = opts.pricing  !== undefined;       // --pricing
  const listAll = opts.all      !== undefined;       // --all (include custom/notes)
  const count   = opts.count    !== undefined;       // --count
  ...
  for (const [key, provider] of Object.entries(MODELS)) {
    ...
    // Filter models by tag or search
    ...
    if (models.length === 0) {
      ...
      models.forEach(m => {
        ...
        // Show model ID on next line (dimmed)
